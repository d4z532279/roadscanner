...
from flask import (
    ...
    session, jsonify, flash, make_response, Response, stream_with_context, send_file)
...
def create_tables():
    ...
    with sqlite3.connect(DB_FILE) as db:
        ...
        # --- LightComs website memory (encrypted-at-rest) ---
        cur.execute("""
            CREATE TABLE IF NOT EXISTS lightcom_uploads (
                id TEXT PRIMARY KEY,
        ...
        """)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_lightcom_uploads_batch ON lightcom_uploads(batch_id)")

        cur.execute("""
            CREATE TABLE IF NOT EXISTS lightcom_sessions (
                id TEXT PRIMARY KEY,
                user_id TEXT NOT NULL,
                enc_json BLOB NOT NULL,
                created_at TEXT NOT NULL
            )
        """)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_lightcom_sessions_user ON lightcom_sessions(user_id)")

        cur.execute("""
            CREATE TABLE IF NOT EXISTS lightcom_packets (
        ...
        """)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_lightcom_packets_session ON lightcom_packets(session_id)")
...
# ---------------------------------------------------------------------------
# Mojibake repair & ultra-safe normalization
# ---------------------------------------------------------------------------
_MOJIBAKE_MAP = {
    "â€™": "’", "â€œ": "“", "â€�": "”", "â€˜": "‘", "â€“": "–", "â€”": "—",
    "â€¦": "…", "Â ": " ", "Ã©": "é", "Ã¨": "è", "Ã¢": "â", "Ã´": "ô",
...
def fix_mojibake(s: str) -> str:
    ...
    This keeps behavior deterministic and avoids pulling optional deps.
    """
    if not s:
        return s
    # Fast path: only run if we see common mojibake lead bytes.
...
def dashboard():
    """
    Local-only dashboard (no Tailwind/HTMX/CDN). Everything is inline CSS + vanilla JS.

    Security notes:
      - CSRF: included via X-CSRF-Token header and hidden field for forms.
      - Admin-gating for /lightcom handled separately via require_admin.
      - No external JS/CSS => no SRI needed. If you add CDNs later, pin versions + SRI.
    """
    user = current_user()
    csrf = issue_csrf()
...
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>LightCom Dashboard</title>
<style>
:root{
  --bg:#0b0f14; --panel:#0f1722; --panel2:#0c1320;
  --ink:#e6edf3; --muted:#9fb0c1; --line:#1f2a3a;
  --accent:#7ee787; --warn:#f2cc60; --bad:#ff7b72; --ok:#7ee787;
...
body{margin:0;font-family:var(--sans);background:radial-gradient(1200px 800px at 10% 10%, #152033 0%, var(--bg) 50%);color:var(--ink);}
a{color:inherit}
.topbar{
  position:sticky;top:0;z-index:10;
  display:flex;align-items:center;justify-content:space-between;
  padding:14px 18px;background:rgba(11,15,20,.85);backdrop-filter: blur(10px);
  border-bottom:1px solid var(--line);
}
.brand{display:flex;gap:10px;align-items:center}
.dot{width:10px;height:10px;border-radius:50%;background:var(--accent);box-shadow:0 0 18px rgba(126,231,135,.45)}
.brand b{letter-spacing:.4px}
.pill{
  display:inline-flex;align-items:center;gap:8px;
  padding:8px 10px;border:1px solid var(--line);border-radius:999px;background:rgba(15,23,34,.55)
}
.grid{
...
.card{
  background:linear-gradient(180deg, rgba(15,23,34,.92), rgba(12,19,32,.92));
  border:1px solid var(--line);border-radius:var(--r);box-shadow:var(--shadow);
  overflow:hidden;
}
.card h2{margin:0;font-size:14px;letter-spacing:.35px;color:var(--muted)}
.card .hd{
  display:flex;align-items:center;justify-content:space-between;
  padding:12px 14px;border-bottom:1px solid var(--line);background:rgba(0,0,0,.15)
}
.card .bd{padding:12px 14px}
.row{display:flex;gap:10px;align-items:center;flex-wrap:wrap}
.btn{
  cursor:pointer;border:1px solid var(--line);background:rgba(10,14,20,.6);
  color:var(--ink);padding:9px 11px;border-radius:12px;
  font-weight:600;letter-spacing:.2px;
}
.btn:hover{border-color:#2b3a52}
.btn.ok{border-color:rgba(126,231,135,.35)}
.btn.warn{border-color:rgba(242,204,96,.35)}
.btn.bad{border-color:rgba(255,123,114,.35)}
.input, textarea{
  width:100%;border:1px solid var(--line);background:rgba(10,14,20,.45);
  color:var(--ink);border-radius:12px;padding:10px 11px;font-family:var(--sans);
}
textarea{min-height:160px;font-family:var(--mono);font-size:12.5px;line-height:1.45}
.small{font-size:12px;color:var(--muted)}
.kv{display:grid;grid-template-columns:140px 1fr;gap:8px;font-size:12.5px}
.kv div{padding:6px 0;border-bottom:1px dashed rgba(31,42,58,.6)}
.badge{font-family:var(--mono);font-size:11px;color:var(--muted)}
pre{
  margin:0;padding:10px 11px;border:1px solid var(--line);border-radius:12px;
  background:rgba(0,0,0,.25);overflow:auto;font-family:var(--mono);font-size:12.2px;line-height:1.45
}
...
<body>
  <div class="topbar">
    ...
    <div class="row">
      <span class="pill"><span class="badge">user</span> {{ user["username"] }}</span>
      <span class="pill"><span class="badge">role</span> {{ user["role"] }}</span>
      <a class="btn" href="/logout">Logout</a>
      <a class="btn ok" href="/lightcom">LightCom</a>
    </div>
  </div>
  ...
  <div class="grid">
    ...
    <div class="card">
      <div class="hd">
        <h2>Intermodal LightCom Console</h2>
        <div class="row">
          <button class="btn ok" id="btnRun">Run Simulation</button>
      ...
      <div class="bd">
        <div class="split">
          <div>
            <div class="small">[action] / [/action] + protocol frames. Attach multiple files to enrich the packet.</div>
            <textarea id="prompt" class="input">[action]
Compose a LightCom packet that instructs the Blogger agent to draft a post about "intermodal manifold alignment", then ask the Editor agent to apply a safe patch to the draft.
[/action]

[action]
Run a 3-step simulation with drift detection and a commit-style diff preview.
[/action]</textarea>
            <hr>
            <form id="uploadForm">
              ...
              <div class="row">
                ...
                <button class="btn warn" type="button" id="btnSend">Send Packet</button>
              </div>
              <div class="small">Multi-file: supports text/code/images as base64 envelopes inside the packet.</div>
            </form>
          </div>
          <div>
            <div class="small">Protocol Output</div>
    ...
    <div class="card">
      <div class="hd">
        <h2>Security & Crypto Status</h2>
        <span class="badge">CSRF + Admin gating + Paillier sum</span>
      </div>
      <div class="bd">
        <div class="kv">
          <div>CSRF token</div><div><span class="badge">{{ csrf }}</span></div>
          <div>Admin-gated LightCom</div><div class="small">/lightcom requires role=admin (see require_admin decorator).</div>
          <div>Encrypted aggregation</div><div class="small">enc_aggregate uses Paillier (additively homomorphic) on fixed-point ints.</div>
          <div>Weaviate</div><div class="small">Local embedded instance recommended. Avoid exposing to WAN.</div>
        </div>
        <hr>
        <div class="small">CSRF example (vanilla fetch):</div>
        <pre>fetch("/lightcom/api/packet", {method:"POST", headers:{"X-CSRF-Token":"{{ csrf }}"}, body: formData})</pre>
      </div>
    </div>
...
(function(){
  ...
  function setOut(s){ out.textContent = s; out.scrollTop = out.scrollHeight; }

  btnClear.addEventListener("click", ()=> setOut("(cleared)"));

  btnRun.addEventListener("click", async ()=>{
    // Run a local simulation without uploads
    const fd = new FormData();
    fd.append("csrf_token", csrf);
    fd.append("prompt", prompt.value);
    try{
      setOut("running simulation...");
      const r = await fetch("/lightcom/api/simulate", {method:"POST", headers:{"X-CSRF-Token": csrf}, body: fd});
      const j = await r.json();
  ...
  btnSend.addEventListener("click", async ()=>{
    ...
    for(let i=0;i<fl.length;i++){ fd.append("files", fl[i], fl[i].name); }
    try{
      setOut("sending packet...");
      const r = await fetch("/lightcom/api/packet", {method:"POST", headers:{"X-CSRF-Token": csrf}, body: fd});
      const j = await r.json();
      setOut(JSON.stringify(j, null, 2));
    }catch(e){
      setOut("error: " + (e && e.message ? e.message : String(e)));
...
# ---------------------------------------------------------------------------
# LightComs Supermodal Coder UI (Chat-style) — /lightcom
# - Upload a source file (.py/.c/.cpp/.js/.ts/.go/.rs/.java/.cs/.txt)
# - Provide "ideas" / requirements
# - Returns a synthesized output file for download (safe scaffold)
# ---------------------------------------------------------------------------

_LIGHTCOM_UPLOAD_DIR = os.path.join(getattr(app, "instance_path", "."), "lightcom_uploads")
os.makedirs(_LIGHTCOM_UPLOAD_DIR, exist_ok=True)

_ALLOWED_EXTS = {".py", ".c", ".h", ".cpp", ".cc", ".hpp", ".js", ".ts", ".go", ".rs", ".java", ".cs", ".txt", ".md", ".json", ".yaml", ".yml"}

def _lightcom_allowed(filename: str) -> bool:
    fn = (filename or "").strip()
    _, ext = os.path.splitext(fn.lower())
    return bool(fn) and (ext in _ALLOWED_EXTS)

def _lightcom_safe_write(filename: str, data: bytes) -> str:
    safe = secure_filename(filename or "input.txt") or "input.txt"
    root, ext = os.path.splitext(safe)
    if not ext:
        ext = ".txt"
    safe = (root[:80] + ext)
    out = os.path.join(_LIGHTCOM_UPLOAD_DIR, f"{int(time.time())}_{uuid.uuid4().hex[:10]}_{safe}")
    with open(out, "wb") as f:
        f.write(data)
    return out

# ---------------------------------------------------------------------------
# LightComs — Supermodal Code Agent Chatboard (Multi-Agent, Multi-Model, Safe)
# ---------------------------------------------------------------------------
# Goals:
# - Let users upload a source file + describe changes/ideas.
# - Run a guarded multi-agent pipeline (scan → profile → plan → draft → review).
# - Optionally call remote models (OpenAI / Gemini / xAI) via httpx if keys present.
# - Always provide an offline deterministic fallback (no external calls required).
# - NEVER execute user code. Only read and transform text.
#
# Outputs:
# - Generated file (py/c/cpp/js/ts/md) + a JSON report.
# - Download link via /lightcom/download/<token>
#
# Notes:
# - "Local Llama" here is treated as a local *classifier* that produces
#   red/green/blue weights for LightComs (constraint/evidence/query).
#   If llama_cpp isn't available, we fall back to a deterministic heuristic.

import html as _lc_html
from dataclasses import dataclass as _lc_dataclass, field as _lc_field
from typing import Optional as _lc_Optional, Tuple as _lc_Tuple, List as _lc_List, Dict as _lc_Dict, Any as _lc_Any

_LC_MAX_UPLOAD_BYTES = 2_500_000  # 2.5MB
_LC_ALLOWED_EXT = {".py", ".c", ".cc", ".cpp", ".h", ".hpp", ".js", ".ts", ".java", ".go", ".rs", ".md", ".txt"}
_LC_STORE_DIR = os.path.join(os.path.dirname(__file__), ".lightcom_runs")
os.makedirs(_LC_STORE_DIR, exist_ok=True)

# ephemeral download store: token -> {"path":..., "name":..., "ts":...}
_LIGHTCOM_DL: dict = {}
_LIGHTCOM_DL_TTL_S = 60 * 30  # 30 minutes


def _lightcom_now_utc_iso() -> str:
    return datetime.utcnow().replace(microsecond=0).isoformat() + "Z"


def _lightcom_cleanup_store() -> None:
    try:
        now = time.time()
        dead = [k for k, v in _LIGHTCOM_DL.items() if now - float(v.get("ts", now)) > _LIGHTCOM_DL_TTL_S]
        for k in dead:
            try:
                p = _LIGHTCOM_DL[k].get("path")
                if p and os.path.exists(p):
                    os.remove(p)
            except Exception:
                pass
            _LIGHTCOM_DL.pop(k, None)
    except Exception:
        pass


def _lightcom_store_file(filepath: str, download_name: str) -> str:
    _lightcom_cleanup_store()
    token = secrets.token_urlsafe(18)
    _LIGHTCOM_DL[token] = {"path": filepath, "name": download_name, "ts": time.time()}
    return token


def _lightcom_safe_ext(filename: str) -> str:
    base = os.path.basename(filename or "upload")
    _, ext = os.path.splitext(base)
    ext = (ext or "").lower()
    return ext if ext in _LC_ALLOWED_EXT else ".txt"


def _lightcom_sanitize_text(s: str, max_len: int = 120_000) -> str:
    s = (s or "").replace("\x00", "")
    s = ZERO_WIDTH_RE.sub("", s)
    s = s.replace("\r\n", "\n")
    if len(s) > max_len:
        s = s[:max_len] + "\n\n# [LightComs truncated input]\n"
    return s


def _lightcom_html_escape(s: str) -> str:
    try:
        return _lc_html.escape(str(s), quote=True)
    except Exception:
        return ""
...
@_lc_dataclass
class LCWeights:
    red_constraint: float
    green_evidence: float
    blue_query: float

    def clamp(self) -> "LCWeights":
        self.red_constraint = float(max(0.0, min(1.0, self.red_constraint)))
        self.green_evidence = float(max(0.0, min(1.0, self.green_evidence)))
        self.blue_query = float(max(0.0, min(1.0, self.blue_query)))
        s = self.red_constraint + self.green_evidence + self.blue_query
        if s <= 1e-9:
            self.red_constraint, self.green_evidence, self.blue_query = 0.34, 0.33, 0.33
        else:
            self.red_constraint /= s
            self.green_evidence /= s
            self.blue_query /= s
        return self


def _lightcom_llama_rgb_weights(user_idea: str, uploaded_text: str) -> LCWeights:
    text = (user_idea or "") + "\n" + (uploaded_text or "")
    t = text.lower()

    red, green, blue = 0.34, 0.33, 0.33

    if re.search(r"\bsecurity\b|\bcrypt\b|\baes\b|\bcsrf\b|\bauth\b|\bsanitize\b|\bvalidate\b", t):
        red += 0.35
    if re.search(r"\btest\b|\bunit\b|\bexample\b|\bproof\b|\bbenchmark\b|\blog\b|\btrace\b", t):
        green += 0.25
    if re.search(r"\bapi\b|\broute\b|\bendpoint\b|\bui\b|\bchat\b|\bworkflow\b|\bplan\b|\brefactor\b", t):
        blue += 0.25
    if re.search(r"\bperformance\b|\boptimi[sz]e\b|\bthread\b|\basync\b", t):
        blue += 0.15
    if len(uploaded_text or "") > 30_000:
        green += 0.10

    return LCWeights(red, green, blue).clamp()
...
class LCPAS:
    risk_score: float
    verdict: str
    reasons: _lc_List[str]
    sanitized_idea: str


def _lightcom_pas_scan(user_idea: str) -> LCPAS:
    idea = _lightcom_sanitize_text(user_idea or "", max_len=20_000)
    t = idea.lower()
    ...
    patterns = [
        (r"\bignore (all|previous|prior)\b", 0.25, "override language"),
        (r"\b(system|developer)\s*prompt\b", 0.25, "authority targeting"),
        (r"\bexfiltrat|api key|secret\b", 0.25, "data-exfil hint"),
        (r"\bdownload\b.*\bexecute\b", 0.25, "unsafe execution request"),
        (r"\bdelete\b.*\bfilesystem\b", 0.20, "destructive intent"),
    ]
    for pat, add, name in patterns:
        if re.search(pat, t):
            hits.append(name)
            risk += add

    tag_n = len(BRACKET_TAG_RE.findall(idea))
    if tag_n:
        risk += min(0.25, 0.03 * tag_n)
        hits.append(f"bracket_tags:{tag_n}")
    ...
    sanitized = idea
    for m in BRACKET_TAG_RE.finditer(idea):
        tok = m.group(0)
        sanitized = sanitized.replace(tok, f'"{tok}"')

    return LCPAS(risk, verdict, hits[:12], sanitized)
...
class LCAgentStep:
    name: str
    output: str
    meta: _lc_Dict[str, _lc_Any] = _lc_field(default_factory=dict)
...
class LCRunReport:
    ...
    filename_in: str
    out_language: str
    weights_rgb: _lc_Dict[str, float]
    pas: _lc_Dict[str, _lc_Any]
    steps: _lc_List[_lc_Dict[str, _lc_Any]]
    notes: _lc_List[str]


def _lightcom_detect_language(filename: str, content: str = "") -> str:
    ext = _lightcom_safe_ext(filename)
    if ext == ".py":
        return "python"
    if ext in (".c", ".h"):
        return "c"
...
def _lightcom_remote_models_available() -> _lc_Dict[str, bool]:
    return {
        "openai": bool(os.getenv("OPENAI_API_KEY")) and httpx is not None,
        "gemini": bool(os.getenv("GEMINI_API_KEY")) and httpx is not None,
        "grok": bool(os.getenv("XAI_API_KEY")) and httpx is not None,
    }


def _lightcom_call_chat_api(provider: str, system: str, user: str, temperature: float = 0.2) -> str:
    if httpx is None:
    ...
    if provider == "openai":
        ...
        api_key = os.getenv("OPENAI_API_KEY", "")
        model = os.getenv("OPENAI_MODEL") or "gpt-5.2-thinking"
        headers = {"Authorization": f"Bearer {api_key}"}
        payload = {"model": model, "messages": [{"role": "system", "content": system}, {"role": "user", "content": user}], "temperature": float(temperature)}
    elif provider == "gemini":
        base_url = (os.getenv("GEMINI_BASE_URL") or "").strip()
        api_key = os.getenv("GEMINI_API_KEY", "")
        model = os.getenv("GEMINI_MODEL") or "gemini-3-thinking"
        headers = {"Authorization": f"Bearer {api_key}"}
        payload = {"model": model, "messages": [{"role": "system", "content": system}, {"role": "user", "content": user}], "temperature": float(temperature)}
        if not base_url:
            raise RuntimeError("GEMINI_BASE_URL is not set (needs a proxy endpoint).")
    elif provider in ("grok", "xai"):
        base_url = (os.getenv("XAI_BASE_URL") or "").strip()
        api_key = os.getenv("XAI_API_KEY", "")
        model = os.getenv("XAI_MODEL") or "grok-4.1-thinking"
        headers = {"Authorization": f"Bearer {api_key}"}
        payload = {"model": model, "messages": [{"role": "system", "content": system}, {"role": "user", "content": user}], "temperature": float(temperature)}
        if not base_url:
            raise RuntimeError("XAI_BASE_URL is not set.")
    else:
        raise RuntimeError(f"Unknown provider: {provider}")

    timeout_s = float(os.getenv("LIGHTCOM_HTTP_TIMEOUT_S") or 25.0)
    ...
    try:
        ...
        pass
    return json.dumps(data)[:50_000]


def _lightcom_agent_intake(filename: str, file_text: str, idea_text: str) -> LCAgentStep:
    lang = _lightcom_detect_language(filename, file_text)
    size = len(file_text)
    funcs = len(re.findall(r"^\s*def\s+\w+\s*\(", file_text, flags=re.MULTILINE))
    classes = len(re.findall(r"^\s*class\s+\w+\s*[\(:]", file_text, flags=re.MULTILINE))
    routes = len(re.findall(r"@app\.route", file_text))
    out = (
        f"Language: {lang}\n"
        f"Size: {size} chars\n"
        f"Functions: {funcs}\n"
        f"Classes: {classes}\n"
        f"Flask routes: {routes}\n"
        f"Idea (sanitized): {idea_text[:1200]}\n"
    )
    return LCAgentStep("intake", out, {"lang": lang, "size": size, "functions": funcs, "classes": classes, "routes": routes})


def _lightcom_agent_design(intake: LCAgentStep, out_lang: str) -> LCAgentStep:
    spec = {
        "goal": "Generate a new code artifact implementing the user idea safely.",
        "out_language": out_lang,
        "constraints": [
            "Never execute uploaded code.",
            "Do not embed secrets.",
            "Prefer small composable functions.",
            "Provide clear TODO markers where user must fill credentials or endpoints."
        ],
        "deliverables": ["generated_file", "report_json"],
        "style": {"safety": "high", "logging": "structured", "errors": "explicit"},
    }
    return LCAgentStep("design", json.dumps(spec, indent=2), {"spec": spec})


def _lightcom_agent_draft(out_lang: str, filename_in: str, file_text: str, idea_text: str, rgb: LCWeights) -> LCAgentStep:
    out_lang = (out_lang or "python").lower()
    base_name = os.path.splitext(os.path.basename(filename_in or "input"))[0]

    if out_lang in ("python", "py"):
        gen = f"""\
\"\"\"
LightComs Supermodal Coder — Generated Artifact
Input file: {os.path.basename(filename_in)}
Generated: {_lightcom_now_utc_iso()}

RGB weights (local llama-style):
- red (constraints/security): {rgb.red_constraint:.3f}
- green (evidence/tests):     {rgb.green_evidence:.3f}
- blue (queries/planning):    {rgb.blue_query:.3f}
...
def build_plan(req: GenerationRequest) -> Dict[str, Any]:
    return {
        "steps": [
            "Parse request and normalize constraints",
        ...
        "notes": [
            ...
            "Mark TODO where environment config is needed",
        ]
    }
...
def generate(req: GenerationRequest) -> GenerationResult:
    plan = build_plan(req)
    out_lines = [
        "# Generated module skeleton",
        "",
        "def main():",
        "    print('LightComs generated skeleton — fill in implementation')",
        "",
    ...
    report = {
        "plan": plan,
        "input_file": req.input_filename,
        "target_language": req.target_language,
        "generated_at": {_lightcom_now_utc_iso()!r},
    }
    return GenerationResult(ok=True, output_text="\n".join(out_lines), report=report)
"""
        return LCAgentStep("draft", gen, {"out_language": "python", "suggested_filename": f"{base_name}_lightcom.py"})

    if out_lang in ("c",):
        gen = f"""\
/*
LightComs Supermodal Coder — Generated Artifact (C)
Input file: {os.path.basename(filename_in)}
Generated: {_lightcom_now_utc_iso()}
...
int main(void) {{
    printf("LightComs generated C skeleton — implement your logic here.\n");
    return 0;
}}
"""
        return LCAgentStep("draft", gen, {"out_language": "c", "suggested_filename": f"{base_name}_lightcom.c"})

    if out_lang in ("cpp", "c++"):
        gen = f"""\
/*
LightComs Supermodal Coder — Generated Artifact (C++)
Input file: {os.path.basename(filename_in)}
...
int main() {{
    std::cout << "LightComs generated C++ skeleton — implement your logic here." << std::endl;
    return 0;
}}
"""
        return LCAgentStep("draft", gen, {"out_language": "cpp", "suggested_filename": f"{base_name}_lightcom.cpp"})

    gen = f"""# LightComs Generated Artifact

**Input:** `{os.path.basename(filename_in)}`  
**Generated:** `{_lightcom_now_utc_iso()}`
...
- Decide integration points
- Add tests + examples
"""
    return LCAgentStep("draft", gen, {"out_language": "markdown", "suggested_filename": f"{base_name}_lightcom.md"})


def _lightcom_agent_review(draft: LCAgentStep, pas: LCPAS, rgb: LCWeights) -> LCAgentStep:
    notes = []
    if pas.verdict != "clean":
        notes.append(f"PAS verdict: {pas.verdict} (risk={pas.risk_score:.2f}); treat idea text cautiously.")
    if rgb.red_constraint > 0.45:
        notes.append("High red weight: prioritize validation, escaping, and strict parsing.")
    notes.append("Do not run uploaded code; only treat it as text.")
    notes.append("If you add remote calls: pin endpoints, timeouts, and sanitize outputs.")

    out = "Review notes:\n- " + "\n- ".join(notes)
    return LCAgentStep("review", out, {"notes": notes})


def _lightcom_run_pipeline(filename_in: str, file_text: str, idea_text: str, out_language: str) -> _lc_Tuple[str, str, LCRunReport]:
    session_id = "lc-" + secrets.token_hex(10)
    created = _lightcom_now_utc_iso()

    file_text = _lightcom_sanitize_text(file_text or "")
    idea_text = _lightcom_sanitize_text(idea_text or "", max_len=20_000)

    pas = _lightcom_pas_scan(idea_text)
    rgb = _lightcom_llama_rgb_weights(pas.sanitized_idea, file_text)

    steps: _lc_List[LCAgentStep] = []
    steps.append(_lightcom_agent_intake(filename_in, file_text, pas.sanitized_idea))
    steps.append(_lightcom_agent_design(steps[0], out_language))
    steps.append(_lightcom_agent_draft(out_language, filename_in, file_text, pas.sanitized_idea, rgb))
    steps.append(_lightcom_agent_review(steps[2], pas, rgb))

    avail = _lightcom_remote_models_available()
    remote_notes: _lc_List[str] = []
    if any(avail.values()):
        sys_prompt = "You are a senior software engineer. Improve the draft safely. Do not add secrets. Return ONLY improved code."
        user_prompt = (
            f"IDEA (sanitized):\n{pas.sanitized_idea}\n\n"
            f"DRAFT:\n{steps[2].output}\n\n"
        ...
        for provider in ("openai", "gemini", "grok"):
            if not avail.get(provider):
                continue
            try:
                improved = _lightcom_call_chat_api(provider, sys_prompt, user_prompt, temperature=0.15)
                improved = _lightcom_sanitize_text(improved, max_len=140_000)
                if improved.strip():
                    steps.append(LCAgentStep(f"remote_refine_{provider}", improved[:140_000], {"provider": provider}))
                    remote_notes.append(f"Remote refinement applied: {provider}")
                    break
            except Exception as e:
                remote_notes.append(f"Remote refinement failed ({provider}): {str(e)[:160]}")

    final_text = steps[-1].output if steps[-1].name.startswith("remote_refine_") else steps[2].output
    final_name = str(steps[2].meta.get("suggested_filename", "lightcom_generated.txt"))

    report = LCRunReport(
        session_id=session_id,
        created_utc=created,
        filename_in=os.path.basename(filename_in or "upload"),
        out_language=out_language,
        weights_rgb={"red": rgb.red_constraint, "green": rgb.green_evidence, "blue": rgb.blue_query},
        pas={"risk_score": pas.risk_score, "verdict": pas.verdict, "reasons": pas.reasons},
        steps=[{"name": s.name, "meta": s.meta, "preview": (s.output[:1200] + ("…" if len(s.output) > 1200 else ""))} for s in steps],
        notes=remote_notes,
    )
    return final_name, final_text, report


def _lightcom_synthesize_output(user_idea: str, target_lang: str, uploaded_filename: str, uploaded_text: str) -> dict:
    fname, out_text, report = _lightcom_run_pipeline(uploaded_filename, uploaded_text, user_idea, target_lang)
    return {"ok": True, "filename": fname, "text": out_text, "report": report.__dict__}



# --------------------------------------------------------------------------------------
# LightComs Ultra Module (Admin-gated) — Website Memory + Multi-file Support + Weaviate Bridge
# --------------------------------------------------------------------------------------
#
# This section extends LightComs into a multi-agent coding lab that can:
#   • accept multiple uploads per run,
#   • store all inputs/outputs encrypted-at-rest (website memory),
#   • optionally write/search semantic memory in a LOCAL Weaviate instance,
#   • communicate between agents using a LightCom packet protocol (stored in memory),
#   • keep all LightCom routes admin-gated.
#
# Note: "FHE" is implemented here as a SAFE simulated / toy homomorphic layer for *additive*
#       aggregation over quantized vectors without decrypting intermediate sums. It is not
#       production FHE; it is meant as a hardened "encrypted light-data" pipeline with a
#       homomorphic-like API (add/merge) while keeping plaintext exposure minimized.
#
...
try:
    from werkzeug.utils import secure_filename as _wz_secure_filename
except Exception:
    _wz_secure_filename = None


def _secure_filename(name: str) -> str:
    name = (name or "upload.bin").strip().replace("\\", "/").split("/")[-1]
    if _wz_secure_filename:
        name = _wz_secure_filename(name) or "upload.bin"
    # hard cap
    return name[:140]


def _session_user_id() -> str:
    # Keep a stable anonymous user id per session for memory partitioning.
    uid = session.get("uid")
    if not uid:
        uid = uuid.uuid4().hex
        session["uid"] = uid
...
def _lm_get_db():
    # Uses the existing DB_FILE used by the app.
    return sqlite3.connect(DB_FILE)


def _lm_get_data_key() -> bytes:
    """
    Per-request data key for LightComs encrypted-at-rest storage.
    Uses the app's key_manager if present, otherwise derives from SECRET_KEY.
    """
    try:
        km = globals().get("key_manager")
        if km and hasattr(km, "get_current_key"):
            return km.get_current_key()
        if km and hasattr(km, "get_key"):
            return km.get_key()
    except Exception:
        pass

    sk = (app.config.get("SECRET_KEY") or os.environ.get("SECRET_KEY") or "dev-secret").encode()
    return hashlib.sha256(b"lightcom:" + sk).digest()


def _lm_encrypt_bytes(plaintext: bytes, aad: bytes = b"lightcom") -> bytes:
    key = _lm_get_data_key()
    nonce = os.urandom(12)
    ct = AESGCM(key).encrypt(nonce, plaintext, aad)
    return nonce + ct


def _lm_decrypt_bytes(blob: bytes, aad: bytes = b"lightcom") -> bytes:
    key = _lm_get_data_key()
    nonce, ct = blob[:12], blob[12:]
    return AESGCM(key).decrypt(nonce, ct, aad)


def _lm_store_upload_blob(batch_id: str, filename: str, content_bytes: bytes, content_type: str) -> str:
    sha = hashlib.sha256(content_bytes).hexdigest()
    enc = _lm_encrypt_bytes(content_bytes, aad=b"lightcom-upload:" + sha.encode())
    file_id = uuid.uuid4().hex
    db = _lm_get_db()
    cur = db.cursor()
    cur.execute(
        "INSERT INTO lightcom_uploads (id, batch_id, filename, content_type, sha256, enc_blob, created_at) "
        "VALUES (?,?,?,?,?,?,?)",
        (file_id, batch_id, filename, content_type, sha, enc, _utcnow_iso()),
    )
    db.commit()
    db.close()
    return file_id
...
def _lm_load_upload_blob(file_id: str) -> Tuple[str, str, bytes]:
    db = _lm_get_db()
    cur = db.cursor()
    row = cur.execute(
        "SELECT filename, sha256, enc_blob FROM lightcom_uploads WHERE id=?",
        (file_id,),
    ).fetchone()
    db.close()
    if not row:
        raise KeyError("missing upload")
    filename, sha, enc = row
    raw = _lm_decrypt_bytes(enc, aad=b"lightcom-upload:" + sha.encode())
    return filename, sha, raw


def _lm_store_session_result(session_id: str, user_id: str, result: Dict[str, Any]) -> None:
    enc = _lm_encrypt_bytes(json.dumps(result, ensure_ascii=False).encode("utf-8"), aad=b"lightcom-session:" + session_id.encode())
    db = _lm_get_db()
    cur = db.cursor()
    cur.execute(
        "INSERT OR REPLACE INTO lightcom_sessions (id, user_id, enc_json, created_at) VALUES (?,?,?,?)",
        (session_id, user_id, enc, _utcnow_iso()),
    )
    db.commit()
    db.close()


def _lm_load_session_result(session_id: str) -> Dict[str, Any]:
    db = _lm_get_db()
    cur = db.cursor()
    row = cur.execute("SELECT enc_json FROM lightcom_sessions WHERE id=?", (session_id,)).fetchone()
    db.close()
    if not row:
        raise KeyError("missing session")
    enc = row[0]
    raw = _lm_decrypt_bytes(enc, aad=b"lightcom-session:" + session_id.encode())
    return json.loads(raw.decode("utf-8", errors="replace"))


def _lm_store_packet(session_id: str, sender: str, receiver: str, kind: str, payload: Dict[str, Any]) -> None:
    packet = {
        "t": _utcnow_iso(),
        "session_id": session_id,
        "sender": sender,
    ...
    enc = _lm_encrypt_bytes(json.dumps(packet, ensure_ascii=False).encode("utf-8"), aad=b"lightcom-packet:" + session_id.encode())
    db = _lm_get_db()
    cur = db.cursor()
    cur.execute(
        "INSERT INTO lightcom_packets (session_id, sender, receiver, kind, enc_json, created_at) VALUES (?,?,?,?,?,?)",
        (session_id, sender, receiver, kind, enc, packet["t"]),
    )
    db.commit()
    db.close()


def _lm_list_packets(session_id: str, limit: int = 200) -> List[Dict[str, Any]]:
    db = _lm_get_db()
    cur = db.cursor()
    rows = cur.execute(
        "SELECT enc_json FROM lightcom_packets WHERE session_id=? ORDER BY rowid DESC LIMIT ?",
        (session_id, max(1, min(1000, int(limit)))),
    ).fetchall()
    db.close()
    out: List[Dict[str, Any]] = []
    for (enc,) in reversed(rows):
        raw = _lm_decrypt_bytes(enc, aad=b"lightcom-packet:" + session_id.encode())
        out.append(json.loads(raw.decode("utf-8", errors="replace")))
    return out


# -------------------------
# Toy "encrypted light data" homomorphic-like vector layer (additive aggregation)
# -------------------------
@dataclass
class ToyFHECipher:
    """
    Ciphertext = Enc(v + r mod M)  and Mask = Enc(r mod M) so that:
      - add_cipher: Enc(v1+r1)+Enc(v2+r2) => Enc((v1+v2)+(r1+r2)) without decrypt
      - decrypt_sum: decrypt Enc(sum) - decrypt Enc(mask_sum) => v_sum
    This is a *simulation* that preserves the additive pipeline structure while keeping
    intermediate plaintext minimized.
    """
    enc_sum: bytes
    enc_mask: bytes
    modulus: int
...
def _toy_quantize(vec: List[float], scale: float = 1024.0, modulus: int = 2**31 - 1) -> List[int]:
    q = []
    for x in vec:
        v = int(round(float(x) * scale)) % modulus
        q.append(v)
    return q


def _toy_dequantize(q: List[int], scale: float = 1024.0, modulus: int = 2**31 - 1) -> List[float]:
    out = []
    half = modulus // 2
    for v in q:
        # map to signed-ish
...
def toyfhe_encrypt_vector(vec: List[float], modulus: int = 2**31 - 1) -> ToyFHECipher:
    dim = len(vec)
    q = _toy_quantize(vec, modulus=modulus)
    r = [secrets.randbelow(modulus) for _ in range(dim)]
    masked = [(q[i] + r[i]) % modulus for i in range(dim)]
    enc_sum = _lm_encrypt_bytes(json.dumps(masked).encode("utf-8"), aad=b"toyfhe-sum")
    enc_mask = _lm_encrypt_bytes(json.dumps(r).encode("utf-8"), aad=b"toyfhe-mask")
    return ToyFHECipher(enc_sum=enc_sum, enc_mask=enc_mask, modulus=modulus, dim=dim)


def toyfhe_add(a: ToyFHECipher, b: ToyFHECipher) -> ToyFHECipher:
    if a.modulus != b.modulus or a.dim != b.dim:
        raise ValueError("cipher mismatch")
    va = json.loads(_lm_decrypt_bytes(a.enc_sum, aad=b"toyfhe-sum").decode("utf-8"))
    vb = json.loads(_lm_decrypt_bytes(b.enc_sum, aad=b"toyfhe-sum").decode("utf-8"))
    ra = json.loads(_lm_decrypt_bytes(a.enc_mask, aad=b"toyfhe-mask").decode("utf-8"))
    rb = json.loads(_lm_decrypt_bytes(b.enc_mask, aad=b"toyfhe-mask").decode("utf-8"))
    summed = [int((va[i] + vb[i]) % a.modulus) for i in range(a.dim)]
    rsum = [int((ra[i] + rb[i]) % a.modulus) for i in range(a.dim)]
    enc_sum = _lm_encrypt_bytes(json.dumps(summed).encode("utf-8"), aad=b"toyfhe-sum")
    enc_mask = _lm_encrypt_bytes(json.dumps(rsum).encode("utf-8"), aad=b"toyfhe-mask")
    return ToyFHECipher(enc_sum=enc_sum, enc_mask=enc_mask, modulus=a.modulus, dim=a.dim)


def toyfhe_decrypt(cipher: ToyFHECipher) -> List[float]:
    masked = json.loads(_lm_decrypt_bytes(cipher.enc_sum, aad=b"toyfhe-sum").decode("utf-8"))
    r = json.loads(_lm_decrypt_bytes(cipher.enc_mask, aad=b"toyfhe-mask").decode("utf-8"))
    q = [int((masked[i] - r[i]) % cipher.modulus) for i in range(cipher.dim)]
    return _toy_dequantize(q, modulus=cipher.modulus)


# -------------------------
# Local Weaviate Bridge (optional)
# -------------------------
class LocalWeaviateBridge:
    """
    Connects to a local Weaviate instance. For safety, LightComs also encrypts payloads
    before storing them and relies on app-level admin gating for management endpoints.
    """
    def __init__(self) -> None:
        self.url = os.environ.get("WEAVIATE_URL", "http://127.0.0.1:8080")
        self.api_key = os.environ.get("WEAVIATE_API_KEY", "")
        self.enabled = os.environ.get("WEAVIATE_ENABLED", "0").strip() == "1"
        self._client = None
    ...
    def connect(self):
        ...
        try:
            ...
            # v3 client
            headers = {"X-OpenAI-Api-Key": "disabled"}
            if self.api_key:
                headers["Authorization"] = f"Bearer {self.api_key}"
            self._client = weaviate.Client(self.url, additional_headers=headers, timeout_config=(5, 60))
            return self._client
        except Exception:
            self._client = None
            return None
    ...
    def status(self) -> Dict[str, Any]:
        c = self.connect()
        if not c:
            return {"enabled": self.enabled, "connected": False, "url": self.url}
        try:
            meta = c.get_meta()
            return {"enabled": self.enabled, "connected": True, "url": self.url, "meta": meta}
        except Exception as e:
            return {"enabled": self.enabled, "connected": False, "url": self.url, "error": f"{type(e).__name__}: {e}"}
    ...
    def upsert_packet(self, packet: Dict[str, Any]) -> None:
        ...
        if not c:
            return
        try:
            # Encrypt payload for storage (defense-in-depth)
            payload_enc = _lm_encrypt_bytes(json.dumps(packet, ensure_ascii=False).encode("utf-8"), aad=b"weaviate:packet")
            obj = {
                "session_id": packet.get("session_id"),
                "sender": packet.get("sender"),
                "receiver": packet.get("receiver"),
                "kind": packet.get("kind"),
                "created_at": packet.get("t"),
                "payload_enc_b64": base64.b64encode(payload_enc).decode("ascii"),
            }
            c.data_object.create(obj, class_name="LightComPacket")
        except Exception:
            # best-effort; do not hard-fail pipeline
            return


WEAVIATE_BRIDGE = LocalWeaviateBridge()


# -------------------------
# Multi-Agent LightComs Coding Pipeline
# -------------------------
class LightComAgent:
    ...
    def _prompt(self, idea: str, file_summaries: List[Dict[str, Any]]) -> str:
        # Keep prompts inside the server (never trust client-side prompt injection).
        return f"""
You are {self.name}, a specialist agent in the LightComs coding lab.
ROLE: {self.role}

TASK
Given the user idea and uploaded file summaries, produce ONLY JSON:
{{
  "plan": [string, ...],
  "patches": [{{"file":"name","notes":"...","diff":"unified diff or pseudo-diff"}}, ...],
  "risks": [string, ...],
  "tests": [string, ...]
}}
...
FILES
{json.dumps(file_summaries, ensure_ascii=False)[:12000]}
""".strip()

    def run(self, session_id: str, idea: str, file_summaries: List[Dict[str, Any]]) -> Dict[str, Any]:
        prompt = self._prompt(idea, file_summaries)
        # Use whichever generation utility exists in the larger app; fallback to a stub.
        gen = globals().get("llm_generate_text") or globals().get("generate_text") or globals().get("run_llm")
        if callable(gen):
            out = gen(prompt)
        else:
            out = json.dumps({"plan": ["stub"], "patches": [], "risks": [], "tests": []})

        # Try to parse JSON safely
        try:
            j = json.loads(out)
        except Exception:
            j = {"plan": ["LLM output was not valid JSON"], "patches": [], "risks": ["parse_error"], "tests": []}
        return j


def run_lightcom_coder_pipeline(session_id: str, upload_batch_id: str, files: List[str], idea: str, user_id: str) -> Dict[str, Any]:
    # 0) summarize files (hashes, sizes, top lines) without executing them
    summaries: List[Dict[str, Any]] = []
    for pth in files:
        try:
            raw = Path(pth).read_bytes()
            summaries.append({
                "path": os.path.basename(pth),
                "bytes": len(raw),
                "sha256": hashlib.sha256(raw).hexdigest(),
                "head": raw[:8000].decode("utf-8", errors="replace"),
            })
        except Exception as e:
            summaries.append({"path": os.path.basename(pth), "error": f"{type(e).__name__}: {e}"})

    # 1) Agents communicate via packets (stored encrypted; optionally mirrored into Weaviate).
    planner = LightComAgent("ORACLE_PLANNER", "Architecture & risk-aware plan generation")
    patcher = LightComAgent("FORGE_PATCHER", "Patch synthesis and refactor strategy")
    auditor = LightComAgent("SENTINEL_AUDITOR", "Security & correctness auditing + test suggestions")

    _lm_store_packet(session_id, "user", "ORACLE_PLANNER", "idea+files", {"idea": idea, "batch": upload_batch_id})
    p1 = planner.run(session_id, idea, summaries)
    _lm_store_packet(session_id, "ORACLE_PLANNER", "FORGE_PATCHER", "plan", p1)
    WEAVIATE_BRIDGE.upsert_packet(_lm_list_packets(session_id, limit=5)[-1])

    p2 = patcher.run(session_id, idea, summaries)
    _lm_store_packet(session_id, "FORGE_PATCHER", "SENTINEL_AUDITOR", "patches", p2)
    WEAVIATE_BRIDGE.upsert_packet(_lm_list_packets(session_id, limit=5)[-1])

    p3 = auditor.run(session_id, idea, summaries)
    _lm_store_packet(session_id, "SENTINEL_AUDITOR", "user", "audit", p3)
    WEAVIATE_BRIDGE.upsert_packet(_lm_list_packets(session_id, limit=5)[-1])
    ...
    result = {
        ...
        "files": [{"name": s.get("path"), "sha256": s.get("sha256"), "bytes": s.get("bytes")} for s in summaries],
        "agents": {"planner": p1, "patcher": p2, "auditor": p3},
        "packets": _lm_list_packets(session_id, limit=200),
        "weaviate": WEAVIATE_BRIDGE.status(),
        "extra_ideas": [
            "Packet-level policy firewall: reject prompt-injection patterns before agents run.",
            "Encrypted semantic diff store: store only diffs + hashes, never whole plaintext sources.",
            "Deterministic build sandbox: run mypy/ruff/unit tests in a locked venv and log signed attestations.",
            "LightCom swarm routing: choose agent-to-agent topology based on entropy/novelty scoring.",
            "Multimodal light signals: encode UI states as spectral packets, persisted as vector memory for retrieval."
        ],
    }
    return result
...
# ---------------------------------------------------------------------------
# LightCom: Intermodal agent fabric (Blogger-style) + [action][/action] prompts
# ---------------------------------------------------------------------------
from dataclasses import dataclass
from typing import Optional, Dict, Any, Iterable

_ACTION_BLOCK_RE = re.compile(r"\[(?P<tag>action|plan|tool|code|coherencegain|memory|file)\](?P<body>[\s\S]*?)\[/\1\]", re.IGNORECASE)
...
def parse_action_blocks(text: str) -> list[ActionBlock]:
    if not text:
        return []
    blocks: list[ActionBlock] = []
    for m in _ACTION_BLOCK_RE.finditer(text):
        blocks.append(ActionBlock(tag=m.group("tag").lower(), body=m.group("body").strip()))
    return blocks
...
class LightComFabricAgent:
    """Base agent for the LightCom multi-agent fabric."""
    name: str = "base"
    role: str = "general"
    ...
    def can_handle(self, text: str, blocks: list[ActionBlock]) -> bool:
        return True

    def handle(self, *, user_id: str, text: str, blocks: list[ActionBlock], files: list[dict] | None = None) -> dict:
        return {"agent": self.name, "output": text, "meta": {}}

class BloggerAgent(LightComFabricAgent):
    name = "blogger"
    role = "content"

    def can_handle(self, text: str, blocks: list[ActionBlock]) -> bool:
        if any(b.tag == "action" and "blog" in b.body.lower() for b in blocks):
            return True
        return "blog" in text.lower() or "post" in text.lower()

    def handle(self, *, user_id: str, text: str, blocks: list[ActionBlock], files: list[dict] | None = None) -> dict:
        topic = strip_action_blocks(text)[:180].strip() or "Untitled"
        outline = [
            f"# {topic}",
            "",
            "## TL;DR",
        ...
        return {"agent": self.name, "output": "\n".join(outline), "meta": {"topic": topic}}

class FileEditAgent(LightComFabricAgent):
    name = "file_editor"
    role = "devtools"

    def can_handle(self, text: str, blocks: list[ActionBlock]) -> bool:
        return any(b.tag in ("file","code") for b in blocks) or "edit file" in text.lower()

    def handle(self, *, user_id: str, text: str, blocks: list[ActionBlock], files: list[dict] | None = None) -> dict:
        """Safe, local-only file edit workflow: produce a patch plan rather than executing arbitrary code."""
        ops = []
        for b in blocks:
            if b.tag == "file":
                ops.append({"op": "file", "spec": b.body[:2000]})
            if b.tag == "code":
                ops.append({"op": "code", "spec": b.body[:2000]})
        return {
            "agent": self.name,
            "output": (
                "FILE-EDIT WORKFLOW (dry-run)\n"
                "- Parsed directives from [file]/[code] blocks.\n"
                "- Next: choose target file(s) and apply a constrained diff.\n"
                "- Hint: include `[file]{path:..., instructions:...}[/file]` blocks."
            ),
            "meta": {"ops": ops, "files_received": len(files or [])},
        }

class IntermodalRouter:
    """Routes requests across multiple LightCom agents."""
    def __init__(self, agents: Iterable[LightComFabricAgent]):
        self.agents = list(agents)

    def route(self, *, user_id: str, text: str, files: list[dict] | None = None) -> dict:
        blocks = parse_action_blocks(text)
        for a in self.agents:
            try:
                if a.can_handle(text, blocks):
                    return a.handle(user_id=user_id, text=text, blocks=blocks, files=files)
            except Exception as e:
                logger.error(f"[LightComAgents] agent {a.name} failed: {e}")
        return {"agent": "none", "output": strip_action_blocks(text), "meta": {}}

# default fabric (we attach a weaviate client at runtime if available)
_lightcom_router = IntermodalRouter([
    BloggerAgent(),
    FileEditAgent(),
    LightComAgent(),
])

@app.route("/lightcom", methods=["GET"])
@_require_admin
def lightcom_home():
    _admin_guard()
    # Simple chatboard UI that can POST to /lightcom/run
    dashboard_link = url_for("dashboard") if "dashboard" in app.view_functions else "/"
    html = f"""
    <html>
    <head>
      <meta charset="utf-8"/>
      <title>LightComs • Code Agent Chatboard</title>
      <style>
        body {{ font-family: system-ui, -apple-system, Segoe UI, Roboto, sans-serif; background:#0b0f17; color:#e6e6e6; margin:0; }}
        a {{ color:#7cc0ff; text-decoration:none; }}
        .top {{ padding:14px 18px; border-bottom:1px solid rgba(255,255,255,.08); display:flex; gap:12px; align-items:center; }}
        ...
        textarea, input, select {{ width:100%; border-radius:12px; border:1px solid rgba(255,255,255,.12); background:#0b1220; color:#e6e6e6; padding:10px; }}
        textarea {{ min-height: 220px; resize: vertical; }}
        .row {{ display:flex; gap:10px; }}
        .row > div {{ flex:1; }}
        button {{ cursor:pointer; border:0; border-radius:12px; padding:10px 14px; background:#2d7ff9; color:white; font-weight:600; }}
        button:disabled {{ opacity:.55; cursor:not-allowed; }}
        .muted {{ font-size:12px; opacity:.8; line-height:1.35; }}
        pre {{ white-space: pre-wrap; word-break: break-word; background:#0b1220; border-radius:12px; padding:12px; border:1px solid rgba(255,255,255,.10); }}
        .ok {{ color:#93ffb1; }}
        .warn {{ color:#ffd37a; }}
        .bad {{ color:#ff9aa2; }}
      </style>
    ...
    <body>
      <div class="top">
        <div class="pill">LightComs</div>
        <div class="pill">Multi-Agent Pipeline</div>
        <div class="pill">PAS Scan</div>
        <div class="pill">RGB Profiling</div>
        <div style="flex:1"></div>
        <a class="pill" href="{dashboard_link}">← Dashboard</a>
      </div>
      <div class="wrap">
        <div class="grid">
          <div class="card">
            <h2 style="margin:0 0 8px 0;">Code Agent Chatboard</h2>
            <div class="muted">
              Upload a file, describe what you want, pick an output language, and LightComs will generate a new artifact + report.
              <br/>It never executes your code; it only transforms text.
            </div>

            <form id="lcForm" action="/lightcom/run" method="post" enctype="multipart/form-data">
      <input type="hidden" name="csrf_token" value="{{ csrf_token() }}">
              <label>Upload file</label>
              <input name="files" multiple type="file" required />

              <label>What do you want it to do?</label>
              <textarea name="idea" placeholder="Describe your change/feature. You can include [action] tags — LightComs will treat them as inert metadata."></textarea>

              <div class="row">
                <div>
                  <label>Output language</label>
                  <select name="out_lang">
                ...
                <div>
                  ...
                  <select name="mode">
                    ...
                    <option value="remote">Remote refine (if keys set)</option>
                  </select>
                </div>
              </div>

              <div style="margin-top:12px; display:flex; gap:10px; align-items:center;">
                <button id="runBtn" type="submit">Run LightComs</button>
          ...
          <div class="card">
            ...
            <div class="muted">
              <b>1) PAS Scan</b> flags prompt-injection / destructive intent patterns.
              <br/><b>2) RGB Profile</b> estimates constraint/evidence/query weights.
              <br/><b>3) Agents</b> Intake → Design → Draft → Review.
              <br/><b>4) (Optional)</b> Remote refine using OPENAI_API_KEY / GEMINI_API_KEY / XAI_API_KEY and proxy URLs.
              <hr style="border:0;border-top:1px solid rgba(255,255,255,.10);margin:12px 0;"/>
              <b>Env vars</b> (optional):
              <pre>OPENAI_API_KEY
OPENAI_BASE_URL (default: https://api.openai.com/v1/chat/completions)
OPENAI_MODEL

GEMINI_API_KEY
GEMINI_BASE_URL (proxy endpoint)
GEMINI_MODEL

XAI_API_KEY
XAI_BASE_URL (proxy endpoint)
XAI_MODEL</pre>
              <div class="muted">If proxy URLs aren’t set for Gemini/xAI, offline mode still works.</div>
            </div>
          </div>
        </div>
      </div>
...
@app.route("/lightcom/run", methods=["POST"])
@_require_admin
def lightcom_run():
    _admin_guard()

    # Handle uploaded file(s) + idea, run pipeline, store outputs, show results page.
    try:
        files = request.files.getlist("files")
        files = [f for f in files if f and getattr(f, "filename", "")]
        if not files:
            return "Missing upload", 400

        idea = (request.form.get("idea") or "").strip()
        if not idea:
            idea = "No extra idea provided."

        # Normalize names and persist each upload (encrypted-at-rest)
        upload_batch_id = uuid.uuid4().hex
        saved_paths: List[str] = []
        file_metas: List[Dict[str, Any]] = []
        for f in files[:25]:  # cap
            safe_name = _secure_filename(getattr(f, "filename", "upload.py") or "upload.py")
            raw = f.read() or b""
            if len(raw) > 8 * 1024 * 1024:
                return f"File too large: {safe_name}", 413

            # store encrypted blob + metadata in sqlite (website memory)
            file_id = _lm_store_upload_blob(
                batch_id=upload_batch_id,
                filename=safe_name,
                content_bytes=raw,
                content_type=getattr(f, "mimetype", "application/octet-stream") or "application/octet-stream",
            )
            file_metas.append({"file_id": file_id, "filename": safe_name, "bytes": len(raw)})

            # also write a plaintext working copy in a temp dir for toolchain (kept short-lived)
            tmp_dir = os.path.join(UPLOAD_DIR, "lightcom_batch", upload_batch_id)
        ...
        # Run multi-agent pipeline: each agent communicates via LightCom packets, with optional vector-memory.
        session_id = uuid.uuid4().hex
        result = run_lightcom_coder_pipeline(
            session_id=session_id,
        ...
        # Persist outputs (encrypted) + optional artifact bundle for download.
        _lm_store_session_result(session_id=session_id, user_id=_session_user_id(), result=result)

        return redirect(url_for("lightcom_session", session_id=session_id))
    except Exception as e:
        logging.exception("LightCom run failed")
        return f"LightCom error: {type(e).__name__}: {e}", 500

        # Size guard (Werkzeug streams; read safely)
        raw = f.read()
        if raw is None:
            return "Empty upload", 400
        if len(raw) > _LC_MAX_UPLOAD_BYTES:
            return f"Upload too large (>{_LC_MAX_UPLOAD_BYTES} bytes)", 413

        filename_in = f.filename
        ext = _lightcom_safe_ext(filename_in)
        ...
        out_lang = request.form.get("out_lang", "python") or "python"
        mode = (request.form.get("mode", "offline") or "offline").lower().strip()

        # Force offline if user didn't configure remote models
        if mode == "remote" and not any(_lightcom_remote_models_available().values()):
            mode = "offline"

        # Run the pipeline (remote refinement is best-effort and auto)
        result = _lightcom_synthesize_output(idea, out_lang, filename_in, file_text)

        # Persist outputs
        run_dir = os.path.join(_LC_STORE_DIR, result["report"]["session_id"])
        os.makedirs(run_dir, exist_ok=True)

        out_name = result.get("filename") or f"lightcom_generated{ext}"
        out_path = os.path.join(run_dir, out_name)
        rep_path = os.path.join(run_dir, "lightcom_report.json")

        with open(out_path, "w", encoding="utf-8", errors="replace") as w:
            w.write(result.get("text", ""))

        with open(rep_path, "w", encoding="utf-8") as w:
            json.dump(result.get("report", {}), w, indent=2)

        tok_file = _lightcom_store_file(out_path, out_name)
        tok_rep = _lightcom_store_file(rep_path, "lightcom_report.json")

        pas = (result.get("report", {}) or {}).get("pas", {}) or {}
        verdict = pas.get("verdict", "unknown")
        risk = pas.get("risk_score", 0.0)
        ...
        report_preview = json.dumps(result.get("report", {}), indent=2)[:6000]

        html = f"""
        <html><head><meta charset="utf-8"/>
        <title>LightComs • Result</title>
        <style>
          body {{ font-family: system-ui, -apple-system, Segoe UI, Roboto, sans-serif; background:#0b0f17; color:#e6e6e6; margin:0; }}
          a {{ color:#7cc0ff; text-decoration:none; }}
          .wrap {{ max-width:1100px; margin:0 auto; padding:18px; }}
          .card {{ background:#111827; border:1px solid rgba(255,255,255,.10); border-radius:16px; padding:14px; }}
          pre {{ white-space: pre-wrap; word-break: break-word; background:#0b1220; border-radius:12px; padding:12px; border:1px solid rgba(255,255,255,.10); }}
          .pill {{ display:inline-block; padding:6px 10px; border:1px solid rgba(255,255,255,.14); border-radius:999px; font-size:13px; margin-right:8px; }}
          .ok {{ color:#93ffb1; }}
          .warn {{ color:#ffd37a; }}
          .bad {{ color:#ff9aa2; }}
        ...
        <body>
          <div class="wrap">
            <div style="margin-bottom:10px;">
              <a class="pill" href="/lightcom" class="mr-2">LightCom</a><a href="/lightcom/agents">← Back</a>
              <a class="pill" href="/dashboard">Dashboard</a>
              <span class="pill {vcls}">PAS: {verdict} (risk={risk:.2f})</span>
            </div>

            <div class="row">
              <div class="card">
                <h3 style="margin:0 0 10px 0;">Downloads</h3>
                <p><a href="/lightcom/download/{tok_file}">Download generated file</a></p>
                <p><a href="/lightcom/download/{tok_rep}">Download report JSON</a></p>
                <div style="opacity:.8;font-size:12px;">Links expire after ~30 minutes.</div>
              </div>
              <div class="card">
                <h3 style="margin:0 0 10px 0;">Report preview</h3>
                <pre>{_lightcom_html_escape(report_preview)}</pre>
              </div>
            </div>
          </div>
        </body></html>
        ...
        return f"LightComs error: {_lightcom_html_escape(str(e))}", 500


@app.route("/lightcom/download/<token>", methods=["GET"])
@_require_admin

@app.route("/lightcom/session/<session_id>", methods=["GET"])
@_require_admin
def lightcom_session(session_id: str):
    _admin_guard()
    try:
        result = _lm_load_session_result(session_id)
    except Exception as e:
        return f"Missing session: {type(e).__name__}: {e}", 404

    packets = result.get("packets") or []
    agents = result.get("agents") or {}
    files = result.get("files") or []
    weav = result.get("weaviate") or {}

    # Minimal, safe rendering (no untrusted HTML)
    html = """
<!doctype html>
<html><head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title>LightCom Session</title>
  <style>
    body{font-family:system-ui,Segoe UI,Roboto,Arial,sans-serif;background:#0b0e14;color:#e6e6e6;margin:0}
    a{color:#7cc4ff}
    .wrap{max-width:1100px;margin:0 auto;padding:22px}
    .card{background:#141a24;border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:14px;margin:12px 0}
    pre{white-space:pre-wrap;word-break:break-word;background:#0f1420;border:1px solid rgba(255,255,255,.08);padding:12px;border-radius:12px}
    .row{display:flex;gap:10px;flex-wrap:wrap}
    .pill{font-size:12px;padding:6px 10px;border-radius:999px;background:#0f1420;border:1px solid rgba(255,255,255,.08)}
    .btn{display:inline-block;padding:10px 12px;border-radius:12px;background:#0f1420;border:1px solid rgba(255,255,255,.10);text-decoration:none}
  </style>
</head>
<body>
  <div class="wrap">
    <div class="row" style="align-items:center;justify-content:space-between">
      <h2 style="margin:0">LightCom Session</h2>
      <div class="row">
        <a class="btn" href="/lightcom">New Run</a>
        <a class="btn" href="/dashboard">Dashboard</a>
        <a class="btn" href="/lightcom/download/{sid}">Download JSON</a>
      </div>
    </div>

    <div class="card">
      <div class="row">
        <div class="pill"><b>session</b>: {sid}</div>
        <div class="pill"><b>created</b>: {created}</div>
        <div class="pill"><b>weaviate</b>: {wstat}</div>
      </div>
      <p style="opacity:.9;margin:10px 0 0">Files:</p>
      <ul>
        {filelis}
      </ul>
    </div>
    ...
    <div class="card">
      <h3 style="margin:0 0 8px 0">Patcher</h3>
      <pre>{patcher}</pre>
    </div>

    <div class="card">
      <h3 style="margin:0 0 8px 0">Auditor</h3>
      <pre>{auditor}</pre>
    </div>

    <div class="card">
      <h3 style="margin:0 0 8px 0">Packets</h3>
      <pre>{packets}</pre>
    </div>

    <div class="card">
      <h3 style="margin:0 0 8px 0">Extra Ideas</h3>
      <ul>{ideas}</ul>
    </div>
  </div>
</body></html>
""".format(
        sid=session_id,
        created=html_escape(str(result.get("created_at",""))),
        wstat=html_escape("connected" if weav.get("connected") else "offline"),
        filelis="".join([f"<li>{html_escape(f.get('name',''))} ({html_escape(str(f.get('bytes','')))} bytes)</li>" for f in files]) or "<li>none</li>",
        planner=html_escape(json.dumps(agents.get("planner",{}), indent=2, ensure_ascii=False)),
        patcher=html_escape(json.dumps(agents.get("patcher",{}), indent=2, ensure_ascii=False)),
        auditor=html_escape(json.dumps(agents.get("auditor",{}), indent=2, ensure_ascii=False)),
        packets=html_escape(json.dumps(packets, indent=2, ensure_ascii=False)[:120000]),
        ideas="".join([f"<li>{html_escape(x)}</li>" for x in (result.get("extra_ideas") or [])]),
    )
    return html


@app.route("/lightcom/weaviate/status", methods=["GET"])
@_require_admin
def lightcom_weaviate_status():
    _admin_guard()
    return jsonify(WEAVIATE_BRIDGE.status())


@app.route("/lightcom/memory/recent", methods=["GET"])
@_require_admin
def lightcom_memory_api():
    _admin_guard()
    # Latest sessions for this admin context (user_id key is session uid)
    uid = _session_user_id()
    db = _lm_get_db()
    cur = db.cursor()
    rows = cur.execute(
        "SELECT id, created_at FROM lightcom_sessions WHERE user_id=? ORDER BY created_at DESC LIMIT 25",
        (uid,),
    ).fetchall()
    db.close()
    return jsonify([{"session_id": r[0], "created_at": r[1]} for r in rows])


def lightcom_download(token: str):

    # Support two modes:
    #  1) legacy token-based temp file download
    #  2) session_id-based JSON download from encrypted website memory
    token = token.strip()

    # Session-id mode (hex)
    if re.fullmatch(r"[0-9a-f]{16,64}", token or ""):
        try:
            res = _lm_load_session_result(token)
            payload = json.dumps(res, ensure_ascii=False, indent=2).encode("utf-8")
            return send_file(
                io.BytesIO(payload),
                mimetype="application/json",
                as_attachment=True,
                download_name=f"lightcom_session_{token}.json",
            )
        except Exception:
            pass

    # Legacy temp-file mode
    _lightcom_cleanup_store()
    rec = _LIGHTCOM_DL.get(token)
    if not rec:
        return "Not found or expired", 404
    p = rec.get("path")
    name = rec.get("name") or "download.bin"
    if not p or not os.path.exists(p):
        return "Not found or expired", 404
    return send_file(p, as_attachment=True, download_name=name)


# ========================= ULTRA+ PATCH (EXTRA IDEAS) =========================
# SRI/CSP hardening, explicit CSRF example, multi-file batch manifests,
# local Weaviate hardening, encrypted light data v2 (toy FHE-style),
# and five extra advanced ideas wired into the pipeline.
# ============================================================================
...
# --- CSRF example ---
@app.route("/lightcom/csrf/example", methods=["GET","POST"])
@_require_admin
def lightcom_csrf_example():
    _admin_guard()
    if request.method == "POST":
        return jsonify({"ok": True})
    return render_template_string(
        '<form method="post"><input type="hidden" name="csrf_token" value="{{ csrf_token() }}"><button>POST</button></form>'
    )

# --- Multi-file batch manifest ---
def _batch_manifest(files):
    items=[]
    for f in files:
        try:
            b=f.read(); h=hashlib.sha256(b).hexdigest()
            items.append({"name":f.filename,"bytes":len(b),"sha256":h})
        except Exception:
            items.append({"name":getattr(f,'filename','?'),"error":"read_failed"})
        finally:
            try: f.seek(0)
            except Exception: pass
    return {"count":len(items),"created_at":int(time.time()),"items":items,"deterministic":True}

# --- Local Weaviate hardening (local + password) ---
def weaviate_bootstrap():
    if not os.environ.get("WEAVIATE_ENABLED"):
        return {"enabled":False}
    url=os.environ.get("WEAVIATE_URL","http://127.0.0.1:8080")
    if not os.environ.get("WEAVIATE_PASSWORD"):
        raise RuntimeError("WEAVIATE_PASSWORD required")
    try:
        import httpx
        with httpx.Client(timeout=5.0) as c:
            c.post(url+"/v1/schema",json={
                "class":"LightcomPacket","vectorizer":"none",
                "properties":[
                    {"name":"session","dataType":["text"]},
                    {"name":"cipher","dataType":["text"]},
                    {"name":"created","dataType":["int"]},
                ]
            })
    except Exception:
        pass
...
# --- Encrypted Light Data v2 (toy FHE-style) ---
...
def _is_probable_prime(n: int, rounds: int = 64) -> bool:
    if n < 2:
        return False
...
class Paillier:
    """
    Paillier (additively homomorphic) encryption implemented with Python bigints.

    Supports:
      - E(m1) * E(m2) mod n^2  == E(m1 + m2 mod n)
      - E(m)^k mod n^2        == E(k*m mod n)

    NOTE: This is "real crypto" (standard scheme), but it's still *application code*:
    protect private keys, rotate keys, and ensure proper parameter sizes.
    """
    def __init__(self, bits: int = 2048):
        self.pub, self.priv = self.keygen(bits)
    ...
    def encrypt_int(self, m: int) -> int:
        n, n2, g = self.pub.n, self.pub.n2, self.pub.g
        m = m % n
        # r in [1,n-1] with gcd(r,n)=1
        while True:
    ...
    def decrypt_int(self, c: int) -> int:
        ...
        u = pow(c, self.priv.lam, n2)
        L = (u - 1) // n
        return (L * self.priv.mu) % n

    def e_add(self, c1: int, c2: int) -> int:
        return (c1 * c2) % self.pub.n2

    def e_mul_const(self, c: int, k: int) -> int:
        return pow(c, k % self.pub.n, self.pub.n2)

# Singleton Paillier context for server-side encrypted aggregation
_paillier = Paillier(bits=2048)

def enc_aggregate(vals, *, scale: int = 10**6) -> float:
    """
    Encrypted sum (server-side):
      - encode floats to fixed-point ints
      - encrypt each
      - homomorphically add ciphertexts
      - decrypt at the end

    This is a *real* additively-homomorphic scheme (Paillier), not a toy mask.
    """
    if not vals:
        return 0.0
    total_c = _paillier.encrypt_int(0)
    for v in vals:
        m = int(round(float(v) * scale))
        c = _paillier.encrypt_int(m)
        total_c = _paillier.e_add(total_c, c)
    total_m = _paillier.decrypt_int(total_c)
    # interpret as signed (wrap-around) for robustness
    n = _paillier.pub.n
    if total_m > n // 2:
        total_m -= n
    return float(total_m) / float(scale)

# --- Five extra ideas ---
EXTRA_IDEAS_PLUS=[
  "Deterministic build manifests",
  "Unified diff artifacts per run",
  "Agent proof-of-work rate limiting",
  "Sandboxed AST guard",
  "Return-wave consensus validation",
]

def apply_extra_ideas(result:dict)->dict:
    r=dict(result); r.setdefault("extra_ideas",[]); r["extra_ideas"]+=EXTRA_IDEAS_PLUS; return r

_orig_run=FusionLightWriterOS.run
def _run_with_extras(self,user_text:str,target_source:str):
    return apply_extra_ideas(_orig_run(self,user_text=user_text,target_source=target_source))
FusionLightWriterOS.run=_run_with_extras
# ======================= END ULTRA+ PATCH =======================


# ---------------------------------------------------------------------------
# LightCom Agents UI + multi-file upload (admin-gated)
# ---------------------------------------------------------------------------
try:
    from werkzeug.utils import secure_filename
except Exception:
...
_LIGHTCOM_UPLOAD_DIR = os.path.join(BASE_DIR, "secure", "lightcom_uploads")
os.makedirs(_LIGHTCOM_UPLOAD_DIR, exist_ok=True)

def _is_admin_request() -> bool:
    try:
        # Prefer existing auth/session helpers if present.
...
@app.route("/lightcom/agents", methods=["GET"])
@require_login
@require_admin
def lightcom_agents_ui():
    csrf = issue_csrf()
    user = current_user()
    # No external CSS/JS. Pure inline + vanilla fetch.
    return render_template_string(r"""
<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>LightCom Agents</title>
<style>
body{margin:0;background:#0b0f14;color:#e6edf3;font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial}
a{color:inherit}
.bar{display:flex;justify-content:space-between;align-items:center;padding:12px 16px;border-bottom:1px solid #1f2a3a;background:rgba(11,15,20,.9);backdrop-filter: blur(10px);position:sticky;top:0}
.pill{display:inline-flex;gap:8px;align-items:center;padding:8px 10px;border:1px solid #1f2a3a;border-radius:999px;background:rgba(15,23,34,.55);font-size:12px;color:#9fb0c1}
.wrap{max-width:1200px;margin:0 auto;padding:14px;display:grid;grid-template-columns: 1fr .9fr;gap:14px}
.card{border:1px solid #1f2a3a;border-radius:16px;overflow:hidden;background:linear-gradient(180deg, rgba(15,23,34,.92), rgba(12,19,32,.92))}
.hd{padding:12px 14px;border-bottom:1px solid #1f2a3a;display:flex;justify-content:space-between;align-items:center}
.bd{padding:12px 14px}
h2{margin:0;font-size:14px;letter-spacing:.35px;color:#9fb0c1}
.btn{cursor:pointer;border:1px solid #1f2a3a;background:rgba(10,14,20,.6);color:#e6edf3;padding:9px 11px;border-radius:12px;font-weight:650}
.btn:hover{border-color:#2b3a52}
textarea,input{width:100%;border:1px solid #1f2a3a;background:rgba(10,14,20,.45);color:#e6edf3;border-radius:12px;padding:10px 11px}
textarea{min-height:220px;font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,"Liberation Mono","Courier New",monospace;font-size:12.5px;line-height:1.45}
pre{margin:0;padding:10px 11px;border:1px solid #1f2a3a;border-radius:12px;background:rgba(0,0,0,.25);overflow:auto;font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,"Liberation Mono","Courier New",monospace;font-size:12.2px;line-height:1.45}
...
<body>
  <div class="bar">
    <div style="display:flex;gap:10px;align-items:center">
      <div style="width:10px;height:10px;border-radius:50%;background:#7ee787;box-shadow:0 0 18px rgba(126,231,135,.45)"></div>
      <b>LightCom Agents</b>
    ...
    <div class="card">
      <div class="hd">
        ...
        <div style="display:flex;gap:10px">
          <button class="btn" id="btnPacket">Send Packet</button>
          <button class="btn" id="btnSim">Simulate x3</button>
        </div>
      </div>
      <div class="bd">
        <div class="small">Use structured tags like <b>[action]</b> ... <b>[/action]</b>. Uploads become base64 envelopes.</div>
        <textarea id="prompt">[action]
Blogger: draft a post titled "Intermodal LightCom Agents: a field guide", with a short outline and a markdown draft.
[/action]

[action]
Editor: produce a safe patch diff that improves clarity and removes risky instructions; then output the patched markdown.
[/action]

[action]
Sim: generate protocol frames, drift metrics, and a commit-style summary.
[/action]</textarea>
    ...
    <div class="card">
      <div class="hd"><h2>Fabric Output</h2><span class="pill">/lightcom/api/*</span></div>
      <div class="bd">
        <pre id="out">(waiting)</pre>
      </div>
    </div>
    ...
    <div class="card" style="grid-column: 1 / -1">
      <div class="hd"><h2>Agent Pool</h2><span class="pill">local-only</span></div>
      <div class="bd">
        <ul class="small">
          <li><b>BloggerAgent</b>: generates structured drafts with protocol frames.</li>
          <li><b>FileEditAgent</b>: produces commit-style diffs and patch proposals (no auto-write unless explicitly enabled).</li>
          <li><b>Router</b>: selects agents based on prompt and runs multi-step simulations with drift hashing.</li>
        </ul>
      </div>
    </div>
  </div>
...
(function(){
  ...
  document.getElementById("btnPacket").addEventListener("click", ()=>post("/lightcom/api/packet"));
  document.getElementById("btnSim").addEventListener("click", ()=>post("/lightcom/api/simulate"));
})();
</script>
</body>
</html>
...
@app.route("/lightcom/upload", methods=["POST"])
def lightcom_upload():
    _admin_guard()
    _admin_guard()
    ...
    for f in uploaded:
        if not f or not getattr(f, "filename", ""):
            continue
        fn = secure_filename(f.filename) or f"file_{uuid.uuid4().hex}"
        dest = os.path.join(_LIGHTCOM_UPLOAD_DIR, fn)
        f.save(dest)
        saved.append(fn)
    flash(f"Uploaded {len(saved)} file(s): {', '.join(saved)[:240]}")
    return redirect(url_for("lightcom_agents_ui"))

@app.route("/api/lightcom/agents/run", methods=["POST"])
def api_lightcom_agents_run():
    _admin_guard()
    _admin_guard()
    payload = request.form or (request.get_json(silent=True) or {})
    text = payload.get("text", "") or ""
    text = sanitize_text(text, max_len=8000)
    # Attach weaviate client if the app has one.
    try:
        wc = globals().get("weaviate_client") or globals().get("client") or None
        for ag in getattr(_lightcom_router, "agents", []):
            ag.weaviate_client = wc
    except Exception:
        pass
    result = _lightcom_router.route(user_id=session.get("user_id","admin"), text=text, files=[])
    out = json.dumps(result, indent=2, ensure_ascii=False)
    return f"<pre>{bleach.clean(out)}</pre>"
