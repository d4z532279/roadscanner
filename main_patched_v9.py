    ...
    <div class="content">
            <div class="quick-apps">
                ...
                <div class="quick-right">
                    <a class="chip ghost" href="{{ url_for('blog_backup') }}"><i class="fa-solid fa-box-archive"></i> Blog Backup</a>
...
@app.route("/api/x/fetch_user_tweets", methods=["POST"])
def api_x_fetch_user_tweets():
    data = request.get_json(silent=True) or {}
    bearer = (data.get("bearer") or os.environ.get("X_BEARER") or "").strip()
    user_id = str(data.get("user_id") or "").strip()
    max_results = int(data.get("max_results") or 80)
    pagination_token = data.get("pagination_token")
    if not bearer or not user_id:
        return jsonify({"ok": False, "error": "missing bearer or user_id"}), 400
    try:
        payload = asyncio.run(_x_api.fetch_user_tweets(bearer=bearer, user_id=user_id, max_results=max_results, pagination_token=pagination_token))
        rows = _x_api.parse_tweets(payload, src="user")
        _x_store_tweets(rows)
        return jsonify({"ok": True, "count": len(rows), "tweets": rows[:200], "meta": payload.get("meta", {})})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500


@app.route("/api/x/search_recent", methods=["POST"])
def api_x_search_recent():
    data = request.get_json(silent=True) or {}
    bearer = (data.get("bearer") or os.environ.get("X_BEARER") or "").strip()
    ...
    if not bearer or not query:
        return jsonify({"ok": False, "error": "missing bearer or query"}), 400
    try:
        payload = asyncio.run(_x_api.search_recent(bearer=bearer, query=query, max_results=max_results, next_token=next_token))
        rows = _x_api.parse_tweets(payload, src="topic")
        _x_store_tweets(rows)
        return jsonify({"ok": True, "count": len(rows), "tweets": rows[:200], "meta": payload.get("meta", {})})
    except Exception as e:
...
@app.route("/api/auton/x/start", methods=["POST"])
def api_auton_x_start():
    data = request.get_json(silent=True) or {}
    bearer = (data.get("bearer") or os.environ.get("X_BEARER") or "").strip()
    mode = str(data.get("mode") or "fetch_user").strip()
    user_id = str(data.get("user_id") or "").strip()
    query = str(data.get("query") or "").strip()
    interval_s = data.get("interval_s")
    timebox_minutes = data.get("timebox_minutes")
    if not bearer:
        return jsonify({"ok": False, "error": "missing bearer"}), 400
    try:
        _auton_x.start(
            bearer=bearer,
            mode=mode,
            ...
            timebox_minutes=int(timebox_minutes) if timebox_minutes is not None else None,
        )
        return jsonify({"ok": True, "status": _auton_x.status()})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 400


@app.route("/api/auton/x/stop", methods=["POST"])
def api_auton_x_stop():
    _auton_x.stop()
    return jsonify({"ok": True, "status": _auton_x.status()})


@app.route("/api/auton/x/status", methods=["GET"])
def api_auton_x_status():
    return jsonify({"ok": True, "status": _auton_x.status()})


@app.route("/api/auton/x/window", methods=["POST"])
def api_auton_x_window():
    data = request.get_json(silent=True) or {}
    start_hhmm = str(data.get("start") or "").strip()
    end_hhmm = str(data.get("end") or "").strip()
    try:
        _auton_x.set_window(start_hhmm, end_hhmm)
        return jsonify({"ok": True, "status": _auton_x.status()})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 400
...
# ---------------------------------------------------------------------------
# X DASHBOARD (per-user vault + PQ-hybrid sealed secrets + per-user carousel DB)
# ---------------------------------------------------------------------------

X2_BASE_URL = os.environ.get("X_BASE_URL", "https://api.x.com").rstrip("/")
X2_OPENAI_BASE_URL = os.environ.get("OPENAI_BASE_URL", "https://api.openai.com/v1").rstrip("/")
X2_DEFAULT_MODEL = os.environ.get("OPENAI_MODEL", "gpt-4o-mini").strip() or "gpt-4o-mini"
X2_HTTP_TIMEOUT = float(os.environ.get("RGN_X_HTTP_TIMEOUT", "25.0"))
...
def _require_user_id_or_redirect() -> int:
    u = _current_username()
    if not u:
...
def _vault_ctx(user_id: int, k: str) -> dict:
    # Bind ciphertext to user + key name, and roll epoch daily for key derivation separation.
    return {
        "domain": "user_vault",
        "field": f"u{int(user_id)}:{clean_text(k, 64)}",
        "epoch": int(time.time()) // 86400,
    }

def vault_set(user_id: int, k: str, v: str) -> None:
    k = clean_text(k, 64)
    # store as json so we can extend later
    payload = json.dumps({"v": clean_text(v, 8000)}, ensure_ascii=False)
    enc = encrypt_data(payload, ctx=_vault_ctx(user_id, k))
    ...
    con.execute(
        ...
        (int(user_id), k, enc, now_iso()),
    )
    con.commit()
    con.close()

def vault_get(user_id: int, k: str, default: str = "") -> str:
    k = clean_text(k, 64)
    con = create_database_connection()
    row = con.execute("SELECT v_enc FROM user_vault WHERE user_id=? AND k=?", (int(user_id), k)).fetchone()
    con.close()
    if not row or not row[0]:
        return default
    try:
        pt = decrypt_data(str(row[0]), ctx=_vault_ctx(user_id, k))
        obj = json.loads(pt or "{}")
        v = obj.get("v", default)
        return clean_text(v, 8000) if isinstance(v, str) else default
    except Exception:
        return default
...
def _x2_http_client() -> httpx.Client:
    return httpx.Client(timeout=httpx.Timeout(X2_HTTP_TIMEOUT, connect=min(10.0, X2_HTTP_TIMEOUT)))

def x2_fetch_user_tweets(bearer: str, user_id: str, max_results: int = 80, pagination_token: Optional[str] = None) -> dict:
    bearer = clean_text(bearer, 4000)
    user_id = clean_text(user_id, 64)
    url = f"{X2_BASE_URL}/2/users/{user_id}/tweets"
    params = {
        "max_results": int(max(5, min(100, max_results))),
        "tweet.fields": "id,text,created_at,author_id",
        "expansions": "author_id",
    ...
    headers = {"Authorization": f"Bearer {bearer}"}
    with _x2_http_client() as c:
        r = c.get(url, headers=headers, params=params)
        if r.status_code >= 400:
            raise RuntimeError(f"HTTP {r.status_code}: {r.text[:1000]}")
        return r.json()

def x2_search_recent(bearer: str, query: str, max_results: int = 50, next_token: Optional[str] = None) -> dict:
    bearer = clean_text(bearer, 4000)
    q = clean_text(query, 512)
    if not q:
        return {"data": [], "meta": {}}
    url = f"{X2_BASE_URL}/2/tweets/search/recent"
    params: Dict[str, Any] = {
        "query": q,
        "max_results": int(max(10, min(100, max_results))),
        "tweet.fields": "id,text,created_at,author_id",
...
def x2_parse_tweets(payload: dict, src: str = "user") -> List[dict]:
    data = payload.get("data") or []
    includes = payload.get("includes") or {}
    users = includes.get("users") or []
    id_to_user: Dict[str, str] = {}
    ...
    for t in data:
        try:
            ...
            au = str(t.get("author_id", "")) if t.get("author_id") is not None else ""
            author = clean_text(id_to_user.get(au, au), 64)
            ...
            if tid and txt:
                out.append({"tid": tid, "author": author, "created_at": created, "text": txt, "src": clean_text(src, 24)})
        except Exception:
            pass
    return out

def _x2_db_upsert_tweets(user_id: int, rows: List[dict]) -> int:
    if not rows:
        return 0
    con = create_database_connection()
    con.execute("BEGIN")
    for r in rows:
        con.execute(
            "INSERT INTO x2_tweets(user_id,tid,author,created_at,text,src,inserted_at) VALUES(?,?,?,?,?,?,?) "
            ...
            (int(user_id), r.get("tid",""), r.get("author",""), r.get("created_at",""), r.get("text",""), r.get("src",""), now_iso()),
        )
    con.commit()
    # bound per-user storage
    con.execute(
        "DELETE FROM x2_tweets WHERE rowid IN ("
        "SELECT rowid FROM x2_tweets WHERE user_id=? ORDER BY inserted_at DESC LIMIT -1 OFFSET ?"
        ")",
        (int(user_id), int(os.environ.get("RGN_MAX_TWEETS_STORE", "2500"))),
    )
    con.commit()
    con.close()
    return len(rows)

def _x2_db_list_tweets(user_id: int, limit: int = 300) -> List[dict]:
    con = create_database_connection()
    cur = con.execute(
        "SELECT tid,author,created_at,text,src FROM x2_tweets WHERE user_id=? ORDER BY inserted_at DESC LIMIT ?",
        (int(user_id), int(limit)),
    )
    rows = [{"tid": r[0], "author": r[1] or "", "created_at": r[2] or "", "text": r[3] or "", "src": r[4] or ""} for r in cur.fetchall()]
    con.close()
    return rows

def _x2_db_get_label(user_id: int, tid: str) -> Optional[dict]:
    con = create_database_connection()
    row = con.execute(
        "SELECT neg,sar,tone,edu,truth,cool,click,incl,ext,ipm,summary,tags_json,raw_json,model,created_at "
        "FROM x2_labels WHERE user_id=? AND tid=?",
        (int(user_id), clean_text(tid, 64)),
    ).fetchone()
    con.close()
    if not row:
        return None
    return {
        "neg": float(row[0] or 0.0),
        "sar": float(row[1] or 0.0),
        "tone": float(row[2] or 0.0),
        "edu": float(row[3] or 0.0),
        "truth": float(row[4] or 0.0),
        "cool": float(row[5] or 0.0),
        "click": float(row[6] or 0.0),
        "incl": float(row[7] or 0.0),
        "ext": float(row[8] or 0.0),
        "ipm": float(row[9] or 0.0),
        "summary": row[10] or "",
        "tags": json.loads(row[11] or "[]") if isinstance(row[11], str) else [],
        "raw_json": row[12] or "{}",
        "model": row[13] or "",
        "created_at": row[14] or "",
    }

def _x2_db_upsert_label(user_id: int, tid: str, lab: dict, model: str) -> None:
    tid = clean_text(tid, 64)
    tags = lab.get("tags", [])
    if not isinstance(tags, list):
        tags = []
    tags = [clean_text(x, 48) for x in tags[:14] if clean_text(x, 48)]
    ipm = float(lab.get("ipm", 0.0) or 0.0)
    con = create_database_connection()
    con.execute(
        "INSERT INTO x2_labels(user_id,tid,neg,sar,tone,edu,truth,cool,click,incl,ext,ipm,summary,tags_json,raw_json,model,created_at) "
        "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) "
        ...
        "cool=excluded.cool, click=excluded.click, incl=excluded.incl, ext=excluded.ext, ipm=excluded.ipm, "
        ...
        (
            int(user_id), tid,
            float(clamp01(lab.get("neg", 0.0))),
            float(clamp01(lab.get("sar", 0.0))),
            float(clamp01(lab.get("tone", 0.0))),
            float(clamp01(lab.get("edu", 0.0))),
            float(clamp01(lab.get("truth", 0.0))),
            float(clamp01(lab.get("cool", 0.0))),
            float(clamp01(lab.get("click", 0.0))),
            float(clamp01(lab.get("incl", 0.0))),
            float(clamp01(lab.get("ext", 0.0))),
            float(ipm),
            clean_text(lab.get("summary", "") or "", 420),
            json.dumps(tags, ensure_ascii=False),
            json.dumps(lab, ensure_ascii=False),
            clean_text(model, 128),
            now_iso(),
        ),
    )
    con.commit()
...
def _x2_db_unlabeled_ids(user_id: int, limit: int = 12) -> List[str]:
    con = create_database_connection()
    cur = con.execute(
        "SELECT t.tid FROM x2_tweets t LEFT JOIN x2_labels l ON (t.user_id=l.user_id AND t.tid=l.tid) "
        "WHERE t.user_id=? AND l.tid IS NULL ORDER BY t.inserted_at DESC LIMIT ?",
        (int(user_id), int(limit)),
    )
...
def social_safety_quantum_x2(v: dict) -> float:
...
def estimate_dwell_x2(text: str, v: dict, remaining_s: float) -> float:
    t = clean_text(text, 2400)
    wc = max(1, len(t.split()))
    read_s = wc / 3.6
    q = social_safety_quantum_x2(v)
    base = read_s * (0.78 + 0.40 * min(1.0, q / 2.0))
    base = float(max(3.8, min(22.0, base)))
    if remaining_s < 20:
        base = min(base, max(3.8, remaining_s / 4.0))
    return float(max(3.8, min(22.0, base)))

def _x2_safe_json_extract(s: str) -> Optional[dict]:
    if not s:
        return None
    s2 = str(s).strip()
    m = re.search(r"\[REPLYFORMAT\](.+?)\[/REPLYFORMAT\]", s2, re.DOTALL | re.IGNORECASE)
    blob = m.group(1).strip() if m else s2
...
def x2_openai_label(api_key: str, model: str, tweet: dict) -> dict:
    api_key = clean_text(api_key, 4000)
    model = clean_text(model, 128) or X2_DEFAULT_MODEL
    sys = "You output strict JSON only, inside [REPLYFORMAT] tags."
    prompt = (
        "[ACTION]\n"
        "You are a Social Safety Quantum scorer for an X feed.\n"
        "Do not quote hate or extremist propaganda; summarize safely if present.\n"
        "Return ONLY the specified JSON inside [REPLYFORMAT] tags.\n"
        "All scores are floats in [0,1].\n"
        "Fields: neg,sar,tone,edu,truth,cool,click,incl,ext,summary,tags,title\n"
        "[/ACTION]\n"
        f"META={{\"tweet_id\":\"{tweet.get('tid','')}\",\"author\":\"{tweet.get('author','')}\",\"created_at\":\"{tweet.get('created_at','')}\",\"src\":\"{tweet.get('src','')}\"}}\n"
        "TWEET_TEXT:\n"
        f"{clean_text(tweet.get('text',''), 3200)}\n"
        "[REPLYFORMAT]\n"
        "{\n"
        "  \"neg\": 0.0,\n"
        "  \"sar\": 0.0,\n"
    ...
    url = f"{X2_OPENAI_BASE_URL}/chat/completions"
    ...
    with httpx.Client(timeout=httpx.Timeout(30.0, connect=10.0)) as c:
        ...
        if r.status_code >= 400:
            raise RuntimeError(f"OpenAI HTTP {r.status_code}: {r.text[:600]}")
        j = r.json()
        content = (((j.get("choices") or [{}])[0]).get("message") or {}).get("content") or ""
        obj = _x2_safe_json_extract(content) or {"raw": content}
        # normalize
        for k in ("neg","sar","tone","edu","truth","cool","click","incl","ext"):
            obj[k] = float(clamp01(obj.get(k, 0.0)))
        v = {k: obj.get(k, 0.0) for k in ("neg","sar","tone","edu","truth","cool","click","incl","ext")}
        obj["ipm"] = float(social_safety_quantum_x2(v))
        if not isinstance(obj.get("tags"), list):
            obj["tags"] = []
        obj["summary"] = clean_text(obj.get("summary","") or "", 420)
        obj["title"] = clean_text(obj.get("title","") or "", 140)
        return obj

def _x2_build_carousel(user_id: int, timebox_s: float = 7*60.0, limit: int = 140) -> List[dict]:
    tweets = _x2_db_list_tweets(user_id, limit=max(50, min(900, int(limit))))
    items: List[dict] = []
    remaining = float(max(10.0, timebox_s))
    for t in tweets:
        lab = _x2_db_get_label(user_id, t["tid"])
        if not lab:
            continue
        v = {k: float(lab.get(k, 0.0)) for k in ("neg","sar","tone","edu","truth","cool","click","incl","ext")}
        ipm = float(lab.get("ipm", social_safety_quantum_x2(v)))
        if ipm < 0.45:
            continue
        dwell = estimate_dwell_x2(t.get("text",""), v, remaining)
        items.append({
            "tweet": t,
            "label": lab,
            "v": v,
    ...
    items.sort(key=lambda it: (it["ipm"], it["v"].get("truth",0), it["v"].get("edu",0), it["v"].get("incl",0), 1.0-it["v"].get("click",0), 1.0-it["v"].get("neg",0)), reverse=True)
    return items[:max(10, min(240, len(items)))]
...
def x_api_settings():
    ...
    if x_bearer and "••••••" not in x_bearer:
        vault_set(uid, "x_bearer", x_bearer)
    if oai_key and "••••••" not in oai_key:
...
def x_api_fetch():
    ...
    try:
        ...
        n = _x2_db_upsert_tweets(uid, rows)
        return jsonify({"ok": True, "count": n, "meta": payload.get("meta", {})})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
def x_api_label():
    ...
    ids = _x2_db_unlabeled_ids(uid, limit=int(os.environ.get("RGN_LABEL_BATCH", "8")))
    if not ids:
    ...
    try:
        for tid in ids:
            ...
            labeled += 1
        return jsonify({"ok": True, "count": labeled})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
