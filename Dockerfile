...
# Install the `oqs` Python module (bindings). Keep this minimal.
RUN set -euo pipefail \
 && python -m pip install --no-cache-dir --no-deps liboqs-python==0.14.1

# PQ authenticity verification (accepts either raw bytes OR base64 files)
RUN python <<'PY'
...
def load_raw_or_b64(path: pathlib.Path) -> bytes:
    ...
    # Try strict base64 only if it is ASCII-ish and validates cleanly.
    try:
    ...
    try:
        ...
        return data
    # Heuristic: if it decodes to something plausible (non-empty), accept it.
