...
# -------------------------------------------------------------------
# Bootstrap ONLY what's needed to run the PQ signature verification:
# - cffi + pycparser (required by liboqs-python at runtime)
# - liboqs-python (provides the `oqs` module)
# Pinned with hashes so this step is also integrity-checked.
# -------------------------------------------------------------------
RUN set -euo pipefail; \
    python -m pip install --upgrade pip; \
    cat > /tmp/pq_bootstrap.txt <<'REQ' \
cffi==2.0.0 \
...
# -------------------------------------------------------------------
# PQ authenticity verification (now `import oqs` works)
# -------------------------------------------------------------------
RUN set -euo pipefail; \
    python - <<'PY'
