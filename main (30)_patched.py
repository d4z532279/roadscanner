...
def _lightcom_synthesize_output(input_name: str, input_text: str, ideas: str, out_lang: str):
    """
    Return (filename, contents). Replace this with your LightComs multi-agent pipeline.
    This keeps the original file content and prefixes a structured “plan + ideas” header.
    """
    ideas = (ideas or "").strip()
    header_lines = [
        "# LightComs Supermodal Coder Output",
        f"# input: {input_name}",
        f"# out_lang: {out_lang}",
        ...
        "",
        "# === LIGHTCOMS_PLAN ===",
        "# 1) Parse & summarize input file",
        "# 2) Apply requested ideas as a patch set",
        "# 3) Run lint/sanity checks and emit final file",
        "# NOTE: Wire your MultiModelCircuit(PAS→FRS→LPS) + coder agents here.",
        "# =======================",
        "",
    ]
    ideas_block = ""
    if ideas:
        ideas_block = "\n".join([f"# IDEA: {ln}" for ln in ideas.splitlines() if ln.strip()][:200]) + "\n\n"

    body = input_text
    ext = os.path.splitext(input_name)[1].lower()

    if ext in {".c", ".h", ".cpp", ".cc", ".hpp", ".java", ".cs"}:
        header = "/*\n" + "\n".join([ln.replace("# ", "") for ln in header_lines]) + "\n*/\n"
        ideas_c = "/*\n" + "\n".join([ln.replace("# ", "") for ln in ideas_block.splitlines() if ln.strip()]) + "\n*/\n" if ideas_block else ""
        contents = header + ideas_c + body
        out_fn = f"lightcom_output{ext or '.txt'}"
        return out_fn, contents

    contents = "\n".join(header_lines) + ideas_block + body
    out_fn = f"lightcom_output{ext or '.txt'}"
    return out_fn, contents

@app.route("/lightcom", methods=["GET", "POST"])
def lightcom():
    if request.method == "POST":
        ideas = request.form.get("ideas", "")
        out_lang = request.form.get("out_lang", "py")
        upload = request.files.get("code_file")

        if not upload or not getattr(upload, "filename", ""):
            flash("Please upload a file.", "error")
            return redirect(url_for("lightcom"))

        if not _lightcom_allowed(upload.filename):
        ...
        # Persist upload (optional audit/debug)
        _lightcom_safe_write(upload.filename, raw)

        try:
            input_text = raw.decode("utf-8", errors="ignore")
        except Exception:
            input_text = str(raw[:2000])

        # ---- Hook point for your LightComs multi-agent pipeline ----
        # Example (pseudo):
        #   circuit = MultiModelCircuit([...])
        #   consensus = circuit.run(ideas + "\n\n" + input_text, session_id=uuid.uuid4().hex)
        #   coder_output = multi_agent_codegen(consensus, input_text, ideas)
        # ------------------------------------------------------------
        out_fn, out_text = _lightcom_synthesize_output(upload.filename, input_text, ideas, out_lang)

        out_path = _lightcom_safe_write(out_fn, out_text.encode("utf-8", errors="ignore"))
        return send_file(out_path, as_attachment=True, download_name=out_fn, mimetype="text/plain")
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    body {{
      margin: 0;
      background:
        radial-gradient(1200px 600px at 70% 20%, rgba(124,58,237,0.25), transparent 60%),
        radial-gradient(900px 500px at 20% 60%, rgba(34,197,94,0.18), transparent 55%),
        var(--bg);
      color: var(--text);
    ...
    .side {{
      ...
      padding: 18px 16px;
      background: linear-gradient(180deg, rgba(16,24,39,0.9), rgba(15,23,42,0.6));
    }}
    .brand {{
      display:flex;
      align-items:center;
  ...
  <div class="wrap">
    <aside class="side">
      <div class="brand">
        ...
        <div>
          <div>LightCom</div>
          <div class="hint">Supermodal Coder UI — upload + ideas → output file.</div>
        </div>
      </div>
      <div class="nav">
        <a href="{{{{ url_for('dashboard') }}}}">← Back to Dashboard</a>
        <a href="{{{{ url_for('lightcom') }}}}"><span class="pill">/lightcom</span> Refresh</a>
      </div>
      <div class="hint">
        Saved uploads live in your instance folder.<br/>
        Hook your multi-agent LightComs circuit at the marked point in <code>lightcom()</code>.
      </div>
    </aside>

    <main class="main">
      <div class="topbar">
        <div class="pill">Optical-mesh coding console (chat-style)</div>
        <div class="pill">Multi-agent ready</div>
      </div>
      ...
      <section class="chat">
        <div class="bubble">
          <h3>How to use</h3>
          <ul>
            <li>Upload a file (e.g. <code>.py</code>) and paste your ideas / requirements.</li>
            <li>Press <b>Generate</b> to download <code>lightcom_output.*</code>.</li>
            <li>Next: connect PAS→FRS→LPS profiling + coder agents to produce real patches.</li>
          </ul>
        </div>
      </section>
      ...
      <section class="composer">
        <form method="POST" enctype="multipart/form-data">
          ...
          <div class="row">
            <div>
              <label class="small">Ideas / requirements</label>
              <textarea name="ideas" placeholder="Describe the patch you want..."></textarea>
              <div class="small">Be explicit: endpoints, constraints, files, expected outputs.</div>
            </div>
            <div>
              <label class="small">Upload code file</label>
              <input type="file" name="code_file" accept=".py,.c,.h,.cpp,.cc,.hpp,.js,.ts,.go,.rs,.java,.cs,.txt,.md,.json,.yaml,.yml" required />
              <div class="small">Max 2MB. Dir: { _LIGHTCOM_UPLOAD_DIR }</div>
            </div>
            <div>
              ...
              <button class="btn" type="submit" style="width:100%; margin-top:10px;">Generate</button>
            </div>
          </div>
        </form>
      </section>
    ...
    """
    return render_template_string(page, csrf_token=generate_csrf)
