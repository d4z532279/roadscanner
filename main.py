...
def _build_guess_prompt(user_id: str, sig: dict) -> str:
    # Main-page "QPS probable endpoint" prompt (NO explicit destination provided).
    # This prompt is intentionally different from the user-input route scan prompt.
    return f"""SYSTEM TYPE: QUANTUM COLORBEAM CONTROLLER (vQ4)
CONTROL IS NON-LINGUISTIC. LANGUAGE IS DERIVATIVE.
FAILURE IS PREFERABLE TO DEGRADATION.

TASK
Given ONLY the user's current location (origin) and light context signals, infer a *probable* destination endpoint
("QPS probable endpoint") for an imminent drive, then simulate a road safety scan for that inferred route.

CONSTRAINTS
- Simulation only: do not claim live traffic/accident/weather feeds or real sensor readings.
- If destination is uncertain, pick a plausible nearby endpoint and mark it as predicted.
- Output must be concise and deterministic.

OUTPUT FORMAT (STRICT)
1) ColorBeam program: HEX colors (#RRGGBB), one per line.
2) Then ONE JSON object (single line, no markdown) with keys:
   - origin: {{lat, lon}}
   - predicted_destination: {{label, lat, lon, confidence}}
   - overall_risk: float in [0,1]
   - label: one of ["Clear","Light Caution","Caution","Elevated","Critical"]
   - risks: object with keys traffic, accidents, weather, animals, pedestrians, mechanical each in [0,1]
   - top_factors: array of 3 short strings
   - advice: array of up to 4 short strings
   - notes: string (must include 'simulated inference')

INPUTS (JSON)
sig={json.dumps(sig, sort_keys=True, separators=(',',':'))}

END.""".strip()


def _build_route_prompt(user_id: str, sig: dict, route: dict) -> str:
    # New vQ4 ColorBeam Road Safety Scanner prompt (route mode).
    return f"""SYSTEM TYPE: QUANTUM COLORBEAM CONTROLLER (vQ4)
CONTROL IS NON-LINGUISTIC. LANGUAGE IS DERIVATIVE.
FAILURE IS PREFERABLE TO DEGRADATION.

TASK
Simulate a real-world road safety scan for the ROUTE.
Risk dimensions: traffic, accidents, weather, animals, pedestrians, mechanical risk.
...
2) Then ONE JSON object (single line, no markdown) with keys:
   ...
   - route_summary: short string
   - hazards: array of {category, severity, note}
   - advice: 2-4 short bullet strings (<=80 chars each)

CONSTRAINTS
- Use ONLY simulated inference from provided inputs.
- Do NOT invent specific named incidents; speak generically.
...
# -----------------------------
# Private VectorDB (SQLite) for on-device memory anchoring (no external services)
# -----------------------------

class PrivateVectorDB:
    """A tiny private vector store backed by SQLite.

    - Embeddings are deterministic hash-based (no external model).
    - Intended for local personalization / memory anchoring only.
    """
    ...
    def _init_db(self) -> None:
        with sqlite3.connect(self.db_path) as con:
            con.execute("""
                CREATE TABLE IF NOT EXISTS vdb_items (
                    id TEXT PRIMARY KEY,
            ...
            """)
            con.execute("CREATE INDEX IF NOT EXISTS idx_vdb_kind_time ON vdb_items(kind, created_at DESC)")
            con.commit()

    @staticmethod
    def _embed(text: str, dim: int = 64) -> list[float]:
        # Deterministic signed hashing embedding: fast, private, good enough for similarity.
        # NOTE: This is not semantic embedding; it is a privacy-preserving approximation.
        ...
        v = [0.0] * dim
        for t in toks[:256]:
            h = hashlib.blake2s(t.encode("utf-8"), digest_size=16).digest()
            idx = h[0] % dim
            sign = -1.0 if (h[1] & 1) else 1.0
            # small weight by token length
            v[idx] += sign * (1.0 + min(8.0, len(t) / 4.0))
    ...
    def add(self, kind: str, text: str) -> str:
        ...
        with self._lock, sqlite3.connect(self.db_path) as con:
            con.execute(
                "INSERT INTO vdb_items(id, created_at, kind, text, vec) VALUES(?,?,?,?,?)",
                (item_id, int(time.time()), str(kind), str(text), blob),
            )
            con.commit()
        return item_id

    def search(self, kind: str, query: str, top_k: int = 3) -> list[dict]:
        qv = self._embed(query)
        rows: list[tuple[str, int, str, bytes]] = []
        with self._lock, sqlite3.connect(self.db_path) as con:
            cur = con.execute(
                "SELECT id, created_at, text, vec FROM vdb_items WHERE kind=? ORDER BY created_at DESC LIMIT 200",
                (str(kind),),
            )
            rows = cur.fetchall()

        scored = []
        for _id, created_at, text, vec_blob in rows:
            try:
                v = json.loads(vec_blob.decode("utf-8"))
                s = self._cos(qv, v)
            except Exception:
                s = 0.0
            scored.append((s, _id, created_at, text))

        scored.sort(key=lambda x: x[0], reverse=True)
        out = []
        for s, _id, created_at, text in scored[: max(0, int(top_k))]:
            out.append({"id": _id, "score": round(float(s), 4), "created_at": created_at, "text": text})
        return out

# Single private vector DB instance for the process.
_PRIVATE_VDB = PrivateVectorDB(Path("/var/data") / "private_vectordb_pqe.db")



def _qps_predict_destination(uid: str, origin: dict) -> dict:
    """Predict a probable destination endpoint for the *main page* / no-user-input path.

    This is simulation-only: it uses the private SQLite vector store (pqe) to recall
    recent destinations for similar origins. If no history exists, it generates a
    deterministic nearby endpoint (based on uid hash) so the system can still run.
    """
    lat = float(origin.get("lat", 0.0))
    lon = float(origin.get("lon", 0.0))

    # 1) Try to recall a recent similar route from the private VDB
    q = f"origin:{round(lat,5)},{round(lon,5)}"
    hits = _PRIVATE_VDB.search("route", q, top_k=5)
    for h in hits:
        try:
            obj = json.loads(h.get("text", "") or "{}")
            dest = obj.get("dest") or {}
            if "lat" in dest and "lon" in dest:
                return {
                    "dest_lat": float(dest["lat"]),
                    "dest_lon": float(dest["lon"]),
                    "dest_label": str(dest.get("label") or "QPS recalled endpoint"),
                    "source": "pqe_recall",
    ...
    # 2) No memory → deterministic synthetic endpoint near origin (NOT a hardcoded default route)
    hb = hashlib.blake2s(uid.encode("utf-8"), digest_size=8).digest()
    a = (hb[0] / 255.0) * (2 * math.pi)
    r = 0.01 + (hb[1] / 255.0) * 0.02
    dlat = math.cos(a) * r
    dlon = math.sin(a) * r
    return {
        "dest_lat": lat + dlat,
        "dest_lon": lon + dlon,
        "dest_label": "QPS probable endpoint (simulated)",
        "source": "synthetic",
        "score": 0.0,
    }


def _dashboard_destination(uid: str, origin: dict, destination_text: str) -> dict:
    """Simulated geocode for the *dashboard* path (explicit destination text).

    - Tries to recall a matching destination label from the private VDB (pqe).
    - If not found, deterministically generates a nearby endpoint using a hash of
      (uid, origin, destination_text). This avoids any hardcoded default route.
    """
    dest_label = (destination_text or "").strip()[:180] or "Destination (unspecified)"
    lat = float(origin.get("lat", 0.0))
    lon = float(origin.get("lon", 0.0))

    # 1) Recall previous matching destinations for this label (best-effort).
    q = f"dest_label:{dest_label} origin:{round(lat,5)},{round(lon,5)}"
    hits = _PRIVATE_VDB.search("route", q, top_k=8)
    for h in hits:
        try:
            obj = json.loads(h.get("text", "") or "{}")
            dest = obj.get("dest") or {}
            if str(dest.get("label", "")).strip().lower() == dest_label.lower():
                if "lat" in dest and "lon" in dest:
                    return {
                        "dest_lat": float(dest["lat"]),
                        "dest_lon": float(dest["lon"]),
    ...
    # 2) Deterministic synthetic endpoint near origin.
    base = f"{uid}|{round(lat,5)},{round(lon,5)}|{dest_label}".encode("utf-8")
    hb = hashlib.blake2s(base, digest_size=8).digest()
    a = (hb[0] / 255.0) * (2 * math.pi)
    r = 0.01 + (hb[1] / 255.0) * 0.05  # slightly larger than main-page fallback
    dlat = math.cos(a) * r
    dlon = math.sin(a) * r
    return {
        "dest_lat": lat + dlat,
...
# -----------------------------
# vQ4 ColorBeam Road Safety Scanner (Simulated Inference)
# -----------------------------

def _risk_label(score: float) -> str:
    score = max(0.0, min(1.0, float(score)))
    if score <= 0.20: return "Clear"
    if score <= 0.40: return "Light Caution"
    if score <= 0.60: return "Caution"
    if score <= 0.80: return "Elevated"
    return "Critical"

def _category_severity(score01: float) -> str:
    if score01 < 0.20: return "low"
    if score01 < 0.45: return "moderate"
    if score01 < 0.70: return "high"
    return "severe"

def _stable_rng(*parts: str) -> random.Random:
    h = hashlib.blake2b("|".join(parts).encode("utf-8"), digest_size=16).digest()
    seed = int.from_bytes(h, "big")
    return random.Random(seed)

def _colorbeam_program(uid: str, route: dict, sig: dict, iteration: int) -> list[str]:
    # Deterministic ColorBeam program with required structure.
    rng = _stable_rng(uid, json.dumps(route, sort_keys=True, separators=(",", ":")), str(iteration),
                      json.dumps({k: sig.get(k) for k in ("cpu","ram","entropy")}, sort_keys=True, separators=(",", ":")))

    def rand_hex():
        return "#{:02X}{:02X}{:02X}".format(rng.randrange(0,256), rng.randrange(0,256), rng.randrange(0,256))

    tokens: list[str] = []
    tokens.append("#00FF00")  # START

    # 2) ENTANGLED HEADER GROUP (4)
    for _ in range(4):
        tokens.append(rand_hex())

    # 3) HTN ROOT NODE (1)
    tokens.append(rand_hex())

    # 4) HTN SUBTASK NODES (5)
    for _ in range(5):
        tokens.append(rand_hex())

    # 5) DEFERRED-MEASUREMENT MARKERS (2)
    tokens.append("#0000FF")  # marker A
    tokens.append("#FF00FF")  # marker B

    # 6) ENTROPY BUDGET TOKENS (2)
    tokens.append("#222222")
    tokens.append("#444444")

    # 7) CHECKSUM BLOCK
    tokens.append("#777777")
    for _ in range(12):
        tokens.append(rand_hex())
    tokens.append("#777777")

    tokens.append("#000000")  # END
    return tokens

def _simulate_scan(uid: str, sig: dict, route: dict, iteration: int) -> dict:
    # Build a query string for memory anchoring.
    query = f"route:{route.get('lat')},{route.get('lon')}->{route.get('dest_lat')},{route.get('dest_lon')} iter:{iteration}"
    similar = _PRIVATE_VDB.search("scan", query, top_k=3)

    rng = _stable_rng(uid, query, str(sig.get("cpu")), str(sig.get("ram")), str(sig.get("entropy")))

    # Signals -> pseudo risk components (0..1)
    cpu = float(sig.get("cpu", 0.0) or 0.0) / 100.0
    ram = float(sig.get("ram", 0.0) or 0.0) / 100.0
    ent = float(sig.get("entropy", 0.5) or 0.5)
    dist_hint = min(1.0, abs(float(route.get("dest_lat",0.0)) - float(route.get("lat",0.0))) * 12.0
                    + abs(float(route.get("dest_lon",0.0)) - float(route.get("lon",0.0))) * 9.0)

    # Base risks with mild randomness.
    traffic = max(0.0, min(1.0, 0.25*cpu + 0.15*ram + 0.20*dist_hint + rng.random()*0.25))
    accidents = max(0.0, min(1.0, 0.15*traffic + rng.random()*0.35))
    weather = max(0.0, min(1.0, 0.10*dist_hint + (1.0-ent)*0.25 + rng.random()*0.30))
    animals = max(0.0, min(1.0, rng.random()*0.35 + 0.10*(1.0-dist_hint)))
    pedestrians = max(0.0, min(1.0, rng.random()*0.35 + 0.15*dist_hint))
    mechanical = max(0.0, min(1.0, 0.20*cpu + rng.random()*0.25))

    # Memory anchor nudges (very small) if we found similar past scans.
    mem_nudge = 0.0
    if similar:
        mem_nudge = min(0.06, max(0.0, float(similar[0].get("score", 0.0)) * 0.06))

    overall = max(0.0, min(1.0, 0.26*traffic + 0.18*accidents + 0.18*weather + 0.10*animals + 0.14*pedestrians + 0.14*mechanical + mem_nudge))
    label = _risk_label(overall)

    confidence = max(0.35, min(0.95, 0.55 + 0.25*ent - 0.15*abs(cpu-0.35) + rng.random()*0.08))

    hazards = [
        {"category":"traffic", "severity": _category_severity(traffic), "note":"Congestion/merge complexity may vary."},
        {"category":"accidents", "severity": _category_severity(accidents), "note":"Stay alert near intersections and ramps."},
        {"category":"weather", "severity": _category_severity(weather), "note":"Adjust speed for visibility and surface changes."},
        {"category":"animals", "severity": _category_severity(animals), "note":"Watch shoulders at dawn/dusk."},
        {"category":"pedestrians", "severity": _category_severity(pedestrians), "note":"Be cautious near lots, stops, crossings."},
        {"category":"mechanical", "severity": _category_severity(mechanical), "note":"Check tires, lights, and braking feel."},
    ]

    advice = [
        "Increase following distance; scan mirrors often.",
        "Slow earlier for turns and busy entrances.",
        "Assume others may brake suddenly; leave an out.",
    ]
    if label in ("Elevated", "Critical"):
        advice.append("If conditions worsen, consider delaying or rerouting.")

    scan = {
        "overall_risk": round(float(overall), 2),
        "label": label,
        "confidence": round(float(confidence), 2),
        "route_summary": "Simulated scan over a short local route.",
        "hazards": hazards,
        "advice": advice[:4],
        "memory_anchor": {"matches": similar[:3]},
    }

    # Store route + scan in private VDB (pqe)
    try:
        _PRIVATE_VDB.add(
            "route",
            json.dumps(
                {
                    "origin": {"lat": float(route.get("lat")), "lon": float(route.get("lon"))},
                    "dest": {
                        "lat": float(route.get("dest_lat")),
                        "lon": float(route.get("dest_lon")),
                        "label": route.get("dest_label"),
                    },
                },
                sort_keys=True,
                separators=(",", ":"),
    ...
    _PRIVATE_VDB.add("scan", json.dumps(scan, sort_keys=True, separators=(",", ":")))
    return scan

def _vQ4_scan_payload(uid: str, sig: dict, route: dict, iteration: int) -> dict:
    colorbeam = _colorbeam_program(uid, route, sig, iteration)
    scan = _simulate_scan(uid, sig, route, iteration)
    return {
        "colorbeam": colorbeam,
        "scan": scan,
        "server_enriched": {
...
def api_llm_route():
    ...
    # vQ4 scan is simulated inference (no external lookups). ColorBeam first, then scan result.
    payload = _vQ4_scan_payload(uid, sig, route, iteration=0)
    return _attach_cookie(jsonify(payload))
...
def api_stream():
    ...
    # - dest_lat, dest_lon are optional; if absent, we run a local "QPS probable endpoint" prediction (simulation-only)
    try:
    ...
    dest_lat_q = request.args.get("dest_lat", None)
    dest_lon_q = request.args.get("dest_lon", None)
    dest_label = request.args.get("dest_label", None)
    dest_predicted = False

    if dest_lat_q is not None and dest_lon_q is not None:
        try:
            dest_lat = float(dest_lat_q)
            dest_lon = float(dest_lon_q)
        except Exception:
            return jsonify({"error": "dest_lat and dest_lon must be floats"}), 400
    else:
        pred = _qps_predict_destination(uid, {"lat": lat, "lon": lon})
        dest_lat = float(pred["dest_lat"])
        dest_lon = float(pred["dest_lon"])
        dest_label = dest_label or pred.get("dest_label")
        dest_predicted = True
    ...
    def gen():
        for i in range(20):
            sig = _system_signals(uid)
            payload = _vQ4_scan_payload(uid, sig, route, iteration=i)
...
async def scan_debris_for_route(
...
) -> tuple[str, str, str, str, str, str, list[str], dict]:
    ...
    # --- vQ4 Road Safety Scanner (SIMULATION-ONLY) ---
    # Dashboard path: destination is an explicit text field; we deterministically
    # synthesize a destination coordinate near the origin (or recall from pqe).
    uid = f"user:{user_id}"
    origin = {"lat": float(lat), "lon": float(lon)}
    dest = _dashboard_destination(uid, origin, destination)

    # Signals for simulation
    sig = {
        "cpu": float(cpu_usage or 0.0),
        "ram": float(ram_usage or 0.0),
        "entropy": float(_entropy_state.get("rng").random()) if isinstance(_entropy_state.get("rng"), np.random.Generator) else 0.5,
        "vehicle_type": str(vehicle_type or "unknown"),
        "domain": "road_safety",
        "task": "dashboard_scan",
    }
    route = {
        "lat": float(lat),
        "lon": float(lon),
        "dest_lat": float(dest.get("dest_lat", lat)),
        "dest_lon": float(dest.get("dest_lon", lon)),
        "dest_label": str(dest.get("dest_label", destination)),
        "source": str(dest.get("source", "synthetic")),
        "vehicle_type": str(vehicle_type or "unknown"),
        "street_name": street_name,
    }

    payload = _vQ4_scan_payload(uid, sig, route, iteration=0)
    colorbeam = payload.get("colorbeam") or []
    scan = payload.get("scan") or {}

    # Store a readable report (ColorBeam first, then scan JSON) for the dashboard "View" modal.
    report = (
        "--- COLORBEAM ---\n"
        + "\n".join(colorbeam)
        + "\n\n--- SCAN ---\n"
        ...
        + "\n\n--- META ---\n"
        + json.dumps({"street_name": street_name, "destination": destination, "dest_inferred": dest}, indent=2, sort_keys=True)
    ).strip()

    model_used = "simulation_only"

    logger.debug("Exiting scan_debris_for_route (vQ4 simulation) with model_used=%s", model_used)
    return (
        report,
        f"{cpu_usage}",
        f"{ram_usage}",
...
async def start_scan_route():
    ...
    result, cpu_usage, ram_usage, quantum_results, street_name, model_used, colorbeam, scan = await scan_debris_for_route(
        lat_float,
        lon_float,
        vehicle_type,
        destination,
