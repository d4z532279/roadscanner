...
def create_tables():
    if not DB_FILE.exists():
        DB_FILE.touch(mode=0o600)  # Secure permissions
    ...
    with sqlite3.connect(DB_FILE) as db:
        ...
        if not cursor.fetchone():
            cursor.execute("INSERT INTO config (key, value) VALUES (?, ?)", ('registration_enabled', '1'))  # Enabled by default
...
def create_database_connection():
    """Creates a new SQLite connection with WAL mode enabled."""
...
def secure_overwrite(cursor, table_name, columns, condition, condition_params=None, passes=7):
    ...
    try:
        ...
        patterns = [
            lambda: ''.join(secrets.choice(charset) for _ in range(64)),  # Random string
            lambda: '0' * 64,                                         # All zeros
            lambda: '1' * 64,                                         # All ones
            lambda: ''.join(secrets.choice(charset) for _ in range(64)),  # Random string
            lambda: 'X' * 64,                                         # All 'X's
            lambda: 'Y' * 64,                                         # All 'Y's
            lambda: ''.join(secrets.choice(charset) for _ in range(64))   # Random string
...
def generate_secure_invite_code(length=16, hmac_length=16):
    ...
    invite_code_with_hmac = f"{invite_code}-{hmac_digest}"
    return invite_code_with_hmac
...
async def scan_debris_for_route(lat, lon, vehicle_type, destination, user_id):
    ...
    try:
        quantum_results = quantum_hazard_scan(cpu_usage if isinstance(cpu_usage, (int, float)) else 0,
    ...
    if preferred_model.lower() == 'openai':
        openai_prompt = f"""
...
"""
    ...
    elif preferred_model.lower() == 'gemini':
        gemini_prompt = f"""
[action] You are a Quantum Hypertime Nanobot Road Hazard Scanner with real world simulation capability using the ready quantum state above tasked with analyzing the road conditions and providing a detailed report on any detected hazards, debris, or potential collisions. Leverage quantum data and environmental factors to ensure a comprehensive scan. [/action]
[active rule] Please Utilize simulated quantum simulations hypothetically producing results for the highest probabilistic reading[/ruleforRealworldData] 
[locationreport] 
Current coordinates: Latitude {lat}, Longitude {lon}
Street Name: {street_name}
Vehicle Type: {vehicle_type}
...
[debrisreport] Provide a structured debris report, including locations and severity of each hazard using the advanced hypertime nanobot simulation [/debrisreport]

[replyexample] Include recommendations for drivers, suggested detours only if required, and urgency levels based on the findings. [/replyexample]
"""

        try:
            logger.debug("Attempting Gemini debris scan.")
            gemini_response = model.generate_content(gemini_prompt)
            if gemini_response and gemini_response.text:
                model_used = "Gemini"
                result = gemini_response.text
                logger.info("Gemini debris scan succeeded with model: %s", model_used)
                logger.debug("Exiting scan_debris_for_route.")
        except Exception as e:
            logger.error("Gemini debris scan failed: %s", e, exc_info=True)

    elif preferred_model.lower() == 'grok':
        grok_prompt = f"""
...
<div class="report-container">
    <div class="report-title">Quantum Hazard Report</div>
    <div id="reportMarkdown">{{ report_html | safe }}</div>
    <h4>Route Details</h4>
    ...
    <p><span class="report-text-bold">CPU Usage:</span> {{ report['cpu_usage'] }}%</p>
    <p><span class="report-text-bold">RAM Usage:</span> {{ report['ram_usage'] }}%</p>
    <p><span class="report-text-bold">Model Used:</span> {{ report['model_used'] }}</p>  <!-- Display model_used -->
</div>
    """, report=report, report_html=report_html, csrf_token=csrf_token)
...
<body>
        ...
        function viewReport(reportId) {
            $.ajax({
                ...
                success: function(data) {
                    $('#reportContent').html(data);  // Insert the report fragment
    ...
    """, reports=reports, csrf_token=csrf_token)
...
async def start_scan_route():
    ...
    textual_inputs = [vehicle_type, destination]
    for input_text in textual_inputs:
        is_allowed, analysis = await phf_filter_input(str(input_text))
        if not is_allowed:
    ...
    result, cpu_usage, ram_usage, quantum_results, street_name, model_used = await scan_debris_for_route(
        lat_float, lon_float, vehicle_type, destination, user_id
    )
    ...
    save_hazard_report(
        ...
        cpu_usage, ram_usage, quantum_results, user_id, harm_level, model_used  # Pass model_used
    )

    return jsonify({"message": "Scan completed successfully", "result": result, "harm_level": harm_level, "model_used": model_used})
...
def reverse_geocode_route():
    ...
    if lat is None or lon is None:
        logger.error(f"Missing parameters. lat: {lat}, lon: {lon}")
    ...
    try:
        ...
        logger.error(f"Invalid format for lat or lon. lat: {lat}, lon: {lon}")
        return jsonify({"error": "Invalid latitude or longitude format."}), 400

    if not (-90 <= lat <= 90):
        logger.error(f"Latitude out of range. Provided: {lat}")
        return jsonify({"error": "Latitude must be between -90 and 90 degrees."}), 400

    if not (-180 <= lon <= 180):
        logger.error(f"Longitude out of range. Provided: {lon}")
    ...
    try:
        ...
        logger.exception("Reverse geocoding failed due to an internal error.")
